    ...
    async function loadData() {
      ...
      if (currentUser && userProfile) {
        ...
        const hasGenre = Boolean(
          userProfile.genre && userProfile.genre.trim().length > 0,
        );
        const hasRole = Boolean(
          userProfile.role && userProfile.role.trim().length > 0,
        );
        const hasPhone = Boolean(
          userProfile.phone && userProfile.phone.trim().length > 0,
        );
        const hasCompany = Boolean(
          userProfile.company && userProfile.company.trim().length > 0,
        );
        const hasLocation = Boolean(
          userProfile.location && userProfile.location.trim().length > 0,
        );
        const hasTeam = Boolean(
          userProfile.team && userProfile.team.trim().length > 0,
        );
        ...
        try {
          const res = await fetch(config.getApiUrl("/api/admin/users/profile"));
          if (res.ok) {
            ...
            const hasTeam = Boolean(
              profile?.team && profile.team.trim().length > 0,
            );
          ...
          console.error("❌ [PAGE_WELCOME] Erro ao carregar perfil:", {
            error,
          });
      ...
      try {
        const res = await fetch(config.getApiUrl("/api/admin/products"));
        if (res.ok) {
          const data = await res.json();
        ...
        console.error("❌ [PAGE_WELCOME] Erro ao carregar produtos:", {
          error,
        });
      ...
      try {
        const res = await fetch(config.getApiUrl("/api/admin/projects"));
        if (res.ok) {
          const data = await res.json();
        ...
        console.error("❌ [PAGE_WELCOME] Erro ao carregar projetos:", {
          error,
        });
      ...
      try {
        const res = await fetch(config.getApiUrl("/api/admin/contacts"));
        if (res.ok) {
          const data = await res.json();
        ...
        console.error("❌ [PAGE_WELCOME] Erro ao carregar contatos:", {
          error,
        });
      ...
      setSteps([
        {
          ...
          description:
            "Adicione sua função, equipe e demais informações pessoais.",
          link: "/admin/settings",
    ...
    <div className="w-full flex min-h-full flex-col items-center justify-center p-8 text-zinc-600 dark:text-zinc-200">
      <div className="mb-8 max-w-2xl">
        ...
        <p className="mt-1 text-center text-base">
          Siga as etapas abaixo para finalizar a configuração inicial.
        </p>
      </div>
          ...
          <div className="flex flex-col gap-4 p-6">
            {steps.map((item, index) => (
              <Link
                key={index}
                href={item.link}
                className={`group flex items-center gap-x-4 rounded-lg border p-4 transition-colors ${item.completed ? "border-green-200 bg-green-50 hover:bg-green-100 dark:border-green-700 dark:bg-green-900/30 dark:hover:bg-green-800/40" : "border-zinc-200 bg-zinc-50 hover:border-blue-200 hover:bg-blue-50 dark:border-zinc-700 dark:bg-zinc-800/50 dark:hover:border-blue-600 dark:hover:bg-blue-900/30"}`}
              >
                <div>
                  <div
                    className={`flex size-14 items-center justify-center rounded-lg border transition-colors ${item.completed ? "border-green-300 bg-green-300 group-hover:bg-green-400 dark:border-green-600 dark:bg-green-600 dark:group-hover:bg-green-500" : "border-zinc-200 bg-zinc-100 group-hover:border-blue-200 group-hover:bg-blue-100 dark:border-zinc-600 dark:bg-zinc-700 dark:group-hover:border-blue-600 dark:group-hover:bg-blue-800/50"}`}
                  >
                    {item.completed ? (
                      <span className="icon-[lucide--check-check] size-8 shrink-0 text-white"></span>
                    ) : (
                      <span
                        className={`${item.icon} size-8 shrink-0 text-zinc-400 group-hover:text-blue-400`}
                      ></span>
                    )}
                  </div>
                </div>
            ...
            <div className="flex items-center justify-center pt-2 text-center">
              <InputCheckbox
                id="hide-welcome"
                name="hide-welcome"
                label="Não exibir mais as boas-vindas."
                checked={hideWelcome}
                onChange={(e) => setHideWelcome(e.currentTarget.checked)}
              />
            </div>
          </div>
        </div>
