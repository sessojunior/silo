...
interface DependencyManagementOffcanvasProps {
  ...
  convertToProductDependencyItems: (
...
export default function DependencyManagementOffcanvas({
  open,
  onClose,
  dependencies,
  loadingManagement,
  convertToProductDependencyItems,
  convertFromProductDependencyItems,
  handleReorderDependencies,
  openEditItemForm,
  openDeleteDialog,
  openAddItemForm,
}: DependencyManagementOffcanvasProps) {
  const convertToFlatList = (
    deps: ProductDependency[],
    level = 0,
  ): Array<ProductDependency & { level: number }> => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-normal text-zinc-500 dark:text-zinc-400">
              Organize e configure as dependências do produto
            </p>
          </div>
        </div>
      }
        ...
        <div className="flex flex-col gap-4 pb-4 border-b border-dashed border-zinc-200 dark:border-zinc-700">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-medium text-zinc-600 dark:text-zinc-400">
                  {dependencies.length} dependências principais
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="icon-[lucide--git-branch] size-4 text-zinc-600 dark:text-zinc-400" />
                <span className="text-base font-medium text-zinc-600 dark:text-zinc-400">
                  {convertToFlatList(dependencies).length} itens no total
                </span>
              </div>
            </div>
            <Button
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-2">
            <div className="flex items-start gap-3">
              <span className="icon-[lucide--info] size-4 text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
            ...
            {[
              ...
              >
                <div className="flex items-center gap-2 p-3 bg-white border border-zinc-200 dark:border-zinc-700 rounded-lg dark:bg-zinc-800">
                  ...
                  <div
                    className="h-4 bg-zinc-200 dark:bg-zinc-700 rounded flex-1"
        ...
        {!loadingManagement && (
          <div className="flex-1 min-h-0">
            <ProductDependencyMenuBuilder
              ...
              setItems={(newItems) => {
                const convertedDeps =
                  convertFromProductDependencyItems(newItems);
        ...
        {!loadingManagement && dependencies.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-6">
              Comece adicionando as primeiras dependências deste produto.
            </p>
            <Button
              type="button"
              icon="icon-[lucide--plus]"
