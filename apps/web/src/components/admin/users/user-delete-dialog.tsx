  ...
  async function handleDelete() {
    ...
    try {
      ...
      const response = await fetch(
        config.getApiUrl(`/api/admin/users?id=${user.id}`),
        {
          method: "DELETE",
        },
      );
      ...
      if (data.success) {
        ...
        console.error("❌ [COMPONENT_USER_DELETE] Erro ao excluir usuário:", {
          error: data.error,
        });
      ...
      console.error(
        "❌ [COMPONENT_USER_DELETE] Erro inesperado ao excluir usuário:",
        { error },
      );
      toast({
        type: "error",
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
          <div className="flex items-start gap-3">
            <span className="icon-[lucide--alert-triangle] size-5 text-red-600 dark:text-red-400 shrink-0 mt-0.5" />
            <div>
              <h4 className="text-sm font-medium text-red-800 dark:text-red-200 mb-1">
                Atenção! Esta ação é irreversível
              </h4>
              <p className="text-sm text-red-700 dark:text-red-300">
                O usuário será permanentemente removido do sistema. Todos os
                dados associados (problemas, soluções, etc.) serão mantidos, mas
                ficarão sem referência ao usuário.
              </p>
            </div>
          </div>
        </div>
