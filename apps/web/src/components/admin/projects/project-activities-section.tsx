...
export default function ProjectActivitiesSection({
  ...
  const loadActivities = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.error(
          "? [COMPONENT_PROJECT_ACTIVITIES_SECTION] Erro ao carregar atividades:",
          { error: data.error },
        );
  ...
  const getStatusCircle = (status: ProjectActivity["status"]) => {
    ...
    return (
      <div
        className={`size-3 rounded-full ${statusStyles[status as keyof typeof statusStyles]}`}
      />
    );
  };
  ...
  const getPriorityBadge = (priority: ProjectActivity["priority"]) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${priorityStyles[priority as keyof typeof priorityStyles]}`}
      >
        <span
          className={`icon-[lucide--${priorityIcons[priority as keyof typeof priorityIcons]}] size-3`}
        />
        {priority === "low"
          ? "Baixa"
          : priority === "medium"
  ...
  const handleSubmitActivity = async (activityData: ActivityFormData) => {
    try {
      ...
      if (data.success) {
        ...
        console.error(
          "? [COMPONENT_PROJECT_ACTIVITIES_SECTION] Erro ao criar atividade:",
          { error: data.error },
        );
      }
    } catch (error) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <h4 className="font-medium text-zinc-900 dark:text-zinc-100 truncate">
                  Atividades do projeto {project.name}
                </h4>
            ...
            {loading ? (
              ...
              <div className="text-center py-6">
                ...
                <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-3">
                  Nenhuma atividade criada ainda para este projeto.
                </p>
                <Button
                  onClick={handleCreateActivity}
                  className="flex items-center gap-2 text-sm px-3 py-1.5 mx-auto"
                  style="bordered"
                >
                  <span className="icon-[lucide--plus] size-3.5" />
              ...
              <div className="space-y-3">
                {activities.map((activity) => (
                  <div
                    key={activity.id}
                    className="flex items-center gap-4 p-4 bg-white dark:bg-zinc-900 rounded-lg border border-zinc-200 dark:border-zinc-700 hover:shadow-sm transition-shadow group"
                  >
                    {/* Status Circle */}
                    <div className="shrink-0" title={activity.status}>
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            ...
                            <div className="flex items-center gap-2 sm:hidden">
                              <div className="w-12 bg-zinc-200 dark:bg-zinc-700 rounded-full h-1.5">
                                <div
                                  className="bg-linear-to-r from-green-500 to-green-600 h-1.5 rounded-full transition-all duration-300"
                                  style={{ width: "0%" }}
                                />
                              </div>
                        ...
                        <div className="hidden sm:flex items-center gap-4">
                          ...
                          <div className="shrink-0">
                            <div className="flex -space-x-2">
                              <div
                                className="size-6 bg-blue-100 dark:bg-blue-900/30 rounded-full border-2 border-white dark:border-zinc-900 flex items-center justify-center"
                          ...
                          <div className="shrink-0 text-right hidden lg:block">
                            <div className="text-xs text-zinc-500 dark:text-zinc-400">
                              {activity.startDate && (
                                <div>
                                  Início: {formatDate(activity.startDate)}
                                </div>
                              )}
                              {activity.endDate && (
                                <div>Fim: {formatDate(activity.endDate)}</div>
                              )}
                            </div>
                          </div>
                        </div>
