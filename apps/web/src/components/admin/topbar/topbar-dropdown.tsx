  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      <button
        ...
        type="button"
        className="group inline-flex items-center justify-center gap-x-2 rounded-full border border-transparent text-sm font-semibold text-zinc-800 focus:outline-none dark:text-white"
        aria-haspopup="menu"
      ...
      <div
        ...
        className={`absolute right-0 top-full mt-1 z-50 min-w-60 transform rounded-lg border border-zinc-200 bg-white shadow-md transition-opacity duration-300 dark:divide-zinc-700 dark:border-zinc-700 dark:bg-zinc-800 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}
      >
        ...
        <div className="space-y-0.5 p-1.5">
          {account.map((link) => {
            ...
            const isLogout = link.url === "/logout";
            ...
            if (isLogout) {
              return (
                ...
                >
                  <span
                    className={`${link.icon} size-4 shrink-0 text-zinc-400`}
                  />
                  {link.title}
                </button>
              );
            ...
            return (
              <Link
                ...
                href={link.url}
                prefetch={false}
                className={`flex items-center gap-x-3 rounded-lg px-3 py-2 text-base font-medium transition-all duration-300 hover:bg-zinc-100 focus:bg-zinc-100 focus:outline-none dark:hover:bg-zinc-700 dark:hover:text-zinc-300 dark:focus:bg-zinc-700 dark:focus:text-zinc-300 ${isActive ? "bg-zinc-100 dark:bg-zinc-700 dark:text-zinc-200" : "text-zinc-800 dark:text-zinc-400"}`}
              >
                <span
                  className={`${link.icon} size-4 shrink-0 text-zinc-400`}
                />
