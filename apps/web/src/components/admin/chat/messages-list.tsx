...
import { LoadMessagesButton } from "./load-messages-button";
...
export function MessagesList({
  ...
  const chatBgStyles = useMemo(
    () =>
      ({
        "--chat-bg-light": `url('${appConfig.getPublicPath("/images/chat-light.jpg")}')`,
        "--chat-bg-dark": `url('${appConfig.getPublicPath("/images/chat-dark.jpg")}')`,
      } as CSSProperties),
    [],
  );
  ...
  useEffect(() => {
    if (!isAssistantVariant || messages.length === 0) {
      return;
    }
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        {!hasMoreOlderMessages && messages.length > 0 && (
          <div className="flex justify-center pb-4">
            <div className="bg-white/80 dark:bg-zinc-800/80 backdrop-blur-sm border border-zinc-300 dark:border-zinc-700 rounded-full px-4 pt-0.5 pb-1">
              <span className="text-xs uppercase text-zinc-500 dark:text-zinc-400 font-medium">
                Início da conversa
              </span>
            </div>
          </div>
        ...
        {assistantStatusText ? (
          <div className="flex justify-start py-3">
            <div className="rounded-full border border-white/60 bg-white/80 px-4 py-2 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-800/80">
              <LoadingSpinner text={assistantStatusText} size="sm" />
            </div>
          </div>
        ) : null}
        ...
        {/* Espaçador para garantir que a última mensagem não cole no rodapé */}
        <div className={isAssistantVariant ? "h-12 lg:h-16" : "h-4"} />
      </div>

      {/* Botão fixo "Ir para o fim" - canto inferior direito */}
      {!isAssistantVariant && messages.length > 0 && (
