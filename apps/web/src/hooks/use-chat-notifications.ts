...
export function useChatNotifications() {
  ...
  const loadUnreadMessages = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        config.getApiUrl("/api/admin/chat/unread-messages"),
      );
      ...
      if (response.ok) {
        ...
        if (apiResponse.success && apiResponse.data) {
          const payload = apiResponse.data as {
            unreadMessages?: Record<string, ConversationData>;
          };

          setUnreadMessages(payload.unreadMessages || {});
        }
      }
    } catch (error) {
      console.error(
        "❌ [HOOK_CHAT_NOTIFICATIONS] Erro ao carregar mensagens não lidas:",
        { error },
      );
    } finally {
  ...
  const getConversationsWithUnread = useCallback(() => {
    ...
    const conversations = Object.entries(unreadMessages)
