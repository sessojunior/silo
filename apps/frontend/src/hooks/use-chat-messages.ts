...
import { useChat, MESSAGES_PER_PAGE } from "@/context/chat-context";
...
export function useChatMessages({
  ...
  const olderMessagesRemaining = Math.max(
    0,
    totalMessagesCount - targetMessages.length,
  );
  ...
  useEffect(() => {
    if (
      activeTargetId &&
      activeTargetType &&
      activeTargetId !== lastTargetRef.current
    ) {
      ...
      const loadInitialMessages = async () => {
        try {
          ...
          console.error("❌ [USE_CHAT_MESSAGES] Erro no carregamento inicial", {
            error,
          });
        } finally {
          setIsLoading(false);
  ...
  const handleLoadOlderMessages = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "❌ [USE_CHAT_MESSAGES] Erro ao carregar mensagens antigas",
        { error },
      );
    } finally {
      setIsLoadingOlder(false);
  ...
  const handleLoadNewerMessages = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "❌ [USE_CHAT_MESSAGES] Erro ao carregar mensagens recentes",
        { error },
      );
    } finally {
      setIsLoadingNewer(false);
