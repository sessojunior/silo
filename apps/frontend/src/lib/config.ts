...
export const config = {
  ...
  get publicBasePath(): string {
    const rawEnv = process.env.NEXT_PUBLIC_BASE_PATH;
    const raw = rawEnv === undefined ? "/silo" : rawEnv.trim();
  ...
  /**
   * Origem pública do backend Python para uso no browser.
  ...
  /**
   * Origem interna do backend Python para uso no servidor do web.
