...
export function getSafeAssistantPdfFrameSource(source: string): string | null {
  ...
  if (localPath.startsWith("/uploads/serve/")) {
    return localPath.replace("/uploads/serve/", "/api/upload/serve/");
  }

  return localPath;
}

export function resolveAssistantBrowserUrl(source: string): string {
  if (!source.startsWith("/")) {
    return source;
  }

  return config.getPublicPath(source);
}
