...
type AssistantMessageOptions = {
  ...
  assistantArtifacts?: ChatMessage["assistantArtifacts"];
  assistantThinking?: string | null;
  messageId?: string;
};

type AssistantRequestIdempotencyState = {
  threadId: string;
  content: string;
  key: string;
};
...
const buildAssistantMessage = (
  ...
  return {
    ...
    assistantArtifacts: assistantArtifacts ?? null,
...
const mapThreadMessageToChatMessage = (
  ...
  if (message.senderType === "assistant") {
    return buildAssistantMessage(
      message.content,
      {
        assistantGeneration: message.generation,
...
export default function AiAssistantPage() {
  ...
  const assistantRequestIdempotencyKeyRef = useRef<AssistantRequestIdempotencyState | null>(null);
  ...
  const handleAssistantMessage = useCallback(
    async (messageText: string) => {
      ...
      const previousRequest = assistantRequestIdempotencyKeyRef.current;
      const idempotencyKey =
        previousRequest?.threadId === threadId &&
        previousRequest.content === trimmedMessage
          ? previousRequest.key
          : crypto.randomUUID();
      assistantRequestIdempotencyKeyRef.current = {
        threadId,
      ...
      try {
        ...
        const finalizeAssistantResult = (data: AiAssistantMessageResponseDto) => {
          ...
          const assistantMessage = buildAssistantMessage(
            ...
            {
              assistantGeneration: data.generation,
              assistantVisualization: data.visualization,
              assistantArtifacts: data.artifacts,
              assistantThinking: data.thinking ?? currentThinking,
            },
          );
        ...
        const applyAssistantSseAction = (action: AssistantSseAction) => {
          switch (action.type) {
            case "ignore":
              return;
            case "thinking":
        ...
        while (true) {
          ...
          for (const action of parser.pushChunk(chunk)) {
            applyAssistantSseAction(action);
          }
        }

        for (const action of parser.flush()) {
          applyAssistantSseAction(action);
        }
        ...
        if (hasReceivedResult) {
          assistantRequestIdempotencyKeyRef.current = null;
        }
        setIsSending(false);
      }
    },
  ...
  const handleDeleteMessage = useCallback(
    (messageId: string) => {
      ...
      const hasArtifacts =
        affectedMessages.some((m) => Boolean(m.assistantArtifacts?.length)) ||
        affectedMessages.some(
          (m) => m.assistantVisualization?.kind === "image" && !!m.assistantVisualization.src,
        ) ||
        affectedMessages.some((m) => m.assistantVisualization?.kind === "mermaid");
  ...
  const handleDeleteThread = useCallback(
    ...
    const hasArtifacts = threadMessages
      .some((m) =>
        Boolean(m.assistantArtifacts?.length) ||
