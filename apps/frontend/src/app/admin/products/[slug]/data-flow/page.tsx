...
export default function ProductDataFlowPage() {
  ...
  const runMeta: PertRunMeta = useMemo(() => {
    ...
    const allStarts = groups
      .flatMap((g) => g.tasks.map((t) => Date.parse(t.start)))
      .filter(Number.isFinite);
    const allEnds = groups
      .flatMap((g) => g.tasks.map((t) => Date.parse(t.end)))
      .filter(Number.isFinite);
    const allFinished = groups
      .flatMap((g) =>
        g.tasks.map((t) => (t.finishedAt ? Date.parse(t.finishedAt) : NaN)),
      )
      .filter(Number.isFinite);
    const allStarted = groups
      .flatMap((g) =>
        g.tasks.map((t) => (t.startedAt ? Date.parse(t.startedAt) : NaN)),
      )
      .filter(Number.isFinite);
    return {
      productSlug: modelSlug,
      productLabel: (activeSnapshot?.model ?? modelSlug)
        .replace(/[-_]/g, " ")
        .toUpperCase(),
      date,
      turn,
      ...
      actualStartAt:
        allStarted.length > 0
          ? new Date(Math.min(...allStarted)).toISOString()
          : null,
      actualEndAt:
        allFinished.length > 0
          ? new Date(Math.max(...allFinished)).toISOString()
          : null,
  ...
  if (!activeSnapshot) {
    return (
      <div className="flex h-[calc(100dvh-140px)] min-h-0 w-full items-start rounded-lg border border-zinc-200 bg-white p-4 text-zinc-700 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200">
        Nenhum fluxo de dados encontrado para este modelo.
      </div>
    );
  }
