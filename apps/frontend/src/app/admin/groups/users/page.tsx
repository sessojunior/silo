...
export default function UsersPage() {
  ...
  async function fetchUsers() {
    try {
      ...
      if (data.success) {
        ...
        console.error("❌ [PAGE_USERS] Erro ao carregar usuários:", {
          error: data.error,
        });
        toast({
          type: "error",
      ...
      console.error("❌ [PAGE_USERS] Erro inesperado ao carregar usuários:", {
        error,
      });
      toast({
        type: "error",
  ...
  function formatLastLogin(lastLogin: Date | string | null | undefined): string {
  ...
  async function toggleUserStatus(user: UserWithGroup) {
    ...
    try {
      ...
      if (data.success) {
        ...
        console.error("❌ [PAGE_USERS] Erro ao processar usuário:", {
          action,
          error: data.error,
        });
        toast({
      ...
      console.error("❌ [PAGE_USERS] Erro inesperado ao processar usuário:", {
        action,
        error,
      });
      toast({
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              <span
                                className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium whitespace-nowrap ${user.isActive ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"}`}
                              >
                                {user.isActive ? "🟢 Ativo" : "🔴 Inativo"}
                              </span>
                              {user.emailVerified && (
                                <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400 whitespace-nowrap">
                                  ✅ Verificado
                                </span>
                              )}
                            </div>
                        ...
                        {user.groups && user.groups.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {user.groups.map((group) => (
                              <div
                                key={group.groupId}
                                className="flex items-center gap-1 px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded-md"
                              >
                                <div
                                  className="size-4 rounded-full flex items-center justify-center"
                                  style={{
                                    backgroundColor:
                                      group.groupColor || "#6b7280",
                                  }}
                                >
                                  <span
                                    className={`icon-[lucide--${group.groupIcon}] size-2 text-white`}
                                  />
                                </div>
                          ...
                          <div className="flex items-center gap-1 px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded-md">
                            <div
                              className="size-4 rounded-full flex items-center justify-center"
                              style={{
                                backgroundColor: user.groupColor || "#6b7280",
                              }}
                            >
                              <span
                                className={`icon-[lucide--${user.groupIcon}] size-2 text-white`}
                              />
                            </div>
                            <span className="text-xs text-zinc-700 dark:text-zinc-300">
                              {user.groupName}
