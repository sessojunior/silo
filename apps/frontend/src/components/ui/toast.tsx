    ...
    <>
      ...
      {(Object.keys(positionClasses) as ToastPosition[]).map((position) => (
        <div
          key={position}
          className={clsx(
            "fixed z-[1000] flex flex-col gap-3 pointer-events-none",
          ...
          {toasts
            ...
            .map((toast) => (
              <div
                key={toast.id}
                className={clsx(
                  "relative flex items-start gap-3 pointer-events-auto w-80 rounded-lg p-4 text-white shadow-md overflow-hidden",
              ...
              >
                <span
                  className={clsx(toastIcons[toast.type], "size-5 shrink-0")}
                />

                <div className="flex-1 text-sm">
                  <strong className="block mt-px font-semibold leading-tight">
                    {toast.title}
                  </strong>
                  {toast.description && (
