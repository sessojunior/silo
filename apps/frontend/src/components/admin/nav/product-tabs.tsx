...
const DATA_FLOW_TAB_TITLE =
  "Fluxo de dados obtido pelo Kafka REST Proxy. Enquanto o proxy real nao estiver ativo, a tela usa o feed SMNA compartilhado e cai no fallback local apenas se ele falhar.";
...
export default function ProductTabs({
  ...
  const { pipelines: dataFlowSnapshots, loading: isDataFlowLoading } =
    useDataFlowPipelines(isDataFlowPage ? modelSlug : undefined);

  const turns = useMemo(() => {
    const turnsFromKafka = Array.from(
      new Set(
        dataFlowSnapshots
          .map((snapshot) => String(snapshot.turn).trim())
          .filter(Boolean),
      ),
    );
  ...
  const dayTurnOptions = useMemo(() => {
    ...
    const days = (() => {
      const datesFromKafka = Array.from(
        new Set(
          dataFlowSnapshots.map((snapshot) => snapshot.date).filter(Boolean),
        ),
  ...
  const hasValidSelection = dayTurnOptions.some(
  ...
  const selectedStatus = selectedSnapshot
      ...
      <div className="flex gap-x-2">
        ...
        {tabs.map((tab) => (
          <Button
            ...
            title={
              tab.label === "Fluxo de dados" ? DATA_FLOW_TAB_TITLE : undefined
            }
          >
            {tab.label}
          </Button>
