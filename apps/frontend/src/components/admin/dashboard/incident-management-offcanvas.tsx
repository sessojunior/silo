...
export default function IncidentManagementOffcanvas({
  open,
  onClose,
  onIncidentUpdated,
}: Props) {
  ...
  const loadIncidents = async () => {
    ...
    try {
      ...
      console.error(
        "? [COMPONENT_INCIDENT_MANAGEMENT] Erro ao carregar incidentes:",
        { error },
      );
      toast({ type: "error", title: "Erro ao carregar incidentes" });
    } finally {
  ...
  const handleSaveIncident = async (incidentData: Incident) => {
    try {
      const url = config.getApiUrl("/api/admin/incidents");
      ...
      const body = editingIncident
        ...
        : { name: incidentData.name, color: incidentData.color };
      ...
      if (data.success) {
        ...
        toast({
          type: "error",
          title: data.message || "Erro ao salvar incidente",
        });
      }
    } catch (error) {
      console.error(
        "? [COMPONENT_INCIDENT_MANAGEMENT] Erro ao salvar incidente:",
        { error },
      );
  ...
  const confirmDeleteIncident = async () => {
    ...
    try {
      const response = await fetch(
        config.getApiUrl(`/api/admin/incidents?id=${deletingIncident.id}`),
        {
          method: "DELETE",
        },
      );
      ...
      if (data.success) {
        ...
        toast({
          type: "error",
          title: data.message || "Erro ao excluir incidente",
        });
      }
    } catch (error) {
      console.error(
        "? [COMPONENT_INCIDENT_MANAGEMENT] Erro ao excluir incidente:",
        { error },
      );
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Gerencie os tipos de incidentes que podem ocorrer nos turnos
              </p>
            </div>
              ...
              incidents.map((incident) => (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      style="bordered"
                      className="px-2 py-2 text-red-600 border-red-200 hover:bg-red-50 dark:border-red-800 dark:hover:bg-red-900/20"
          ...
          <div className="mt-6 p-4 bg-blue-50 dark:bg-blue-950/20 rounded-lg">
            <div className="flex items-start gap-3">
              <span className="icon-[lucide--info] size-6 text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
              <div className="text-sm text-blue-800 dark:text-blue-200">
                <p className="font-medium mb-1">Sobre os incidentes:</p>
                <p className="text-blue-700 dark:text-blue-300">
                  Incidentes são usados para categorizar problemas nos turnos.
                  Apenas incidentes reais aparecem nas estatísticas. &quot;Não
                  houve incidentes&quot; não pode ser editado ou excluído.
                  Incidentes em uso não podem ser excluídos.
                </p>
              </div>
            </div>
          </div>
