...
export function ProblemDetailColumn({
  loadingDetail,
  problem,
  solutions,
  images,
              ...
              {problem && solutions.length > 0 && (
                <>
                  <span className="text-sm font-medium">
                    {solutions.length} soluções
                  </span>
                  <span className="text-zinc-300 dark:text-zinc-600"> • </span>
                  <span
                    className="text-sm text-zinc-400"
                    title={`Criado em ${formatDate(new Date(problem.createdAt))}`}
                  >
          ...
          {images.length > 0 &&
            images.map(({ id, image, description }) => (
              <div
                ...
                onClick={() =>
                  onImageClick(toPublicUploadsSrc(image), description)
                }
              >
                <Image
                  src={toPublicUploadsSrc(image)}
                  alt={description}
                  className="h-32 w-auto rounded-lg transition hover:brightness-90"
                  width={200}
                  height={128}
