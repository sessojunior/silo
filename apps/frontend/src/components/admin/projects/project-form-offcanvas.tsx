...
export default function ProjectFormOffcanvas({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // A API exige descrição completa com ao menos 1 caractere.
    if (!formData.description.trim()) {
      toast({
        type: "error",
        title: "Erro na validação",
        description: "Descrição completa é obrigatória",
      });
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6" id="project-form">
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            disabled={saving}
            className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-900 text-base text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50 resize-none dark:placeholder-zinc-500"
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              disabled={saving}
              className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:opacity-50"
            />
          </div>
