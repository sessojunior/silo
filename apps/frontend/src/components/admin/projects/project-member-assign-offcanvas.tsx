...
export default function ProjectMemberAssignOffcanvas({
  isOpen,
  onClose,
  project,
  onSubmit,
}: ProjectMemberAssignOffcanvasProps) {
  ...
  const roleOptions = [
    ...
    { value: "manager", label: "Gerente - Gerenciar tarefas e membros" },
  ...
  const fetchAvailableUsers = useCallback(async () => {
    try {
      ...
      const available = allUsers.filter(
        (user) => !projectMemberIds.includes(user.id) && user.isActive,
      );

      setAvailableUsers(available);
      setFilteredUsers(available);
    } catch (error) {
      console.error("? [PROJECT_MEMBER_ASSIGN] Erro ao carregar usuários:", {
        error,
      });
      toast({
        type: "error",
        ...
        description:
          "Não foi possível carregar a lista de usuários disponíveis.",
      });
    } finally {
      setLoading(false);
    }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      console.error(
        "? [COMPONENT_PROJECT_MEMBER_ASSIGN] Erro ao atribuir membros:",
        { error },
      );
      toast({
        type: "error",
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-zinc-600 dark:text-zinc-400">
                  {search
                    ...
                    : "Todos os usuários ativos já estão atribuídos"}
                </p>
              </div>
            ) : (
              <div className="divide-y divide-zinc-200 dark:divide-zinc-700">
                {filteredUsers.map((user) => (
                  <div
                    key={user.id}
                    className={`p-4 cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800 ${selectedUsers.has(user.id) ? "bg-blue-50 dark:bg-blue-900/20" : ""}`}
                    onClick={() => toggleUserSelection(user.id)}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        {/* Avatar */}
                        <div className="size-10 rounded-full bg-linear-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-semibold">
                      ...
                      {/* Checkbox */}
                      <div
                        className={`size-5 rounded border-2 flex items-center justify-center ${selectedUsers.has(user.id) ? "bg-blue-600 border-blue-600" : "border-zinc-300 dark:border-zinc-600"}`}
                      >
        ...
        {selectedUsers.size > 0 && (
          <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
            ...
            <p className="text-blue-700 dark:text-blue-300 text-sm">
              <strong>{selectedUsers.size}</strong> usuário(s) serão atribuídos
              como{" "}
              <strong>
                {roleOptions.find((r) => r.value === selectedRole)?.label}
              </strong>{" "}
              ao projeto <strong>{project.name}</strong>
            </p>
          </div>
        )}
      </div>
