...
async def delete_incident(
    ...
    if not id:
        return service_error_response(
            service_failure("ID do incidente é obrigatório.", 400),
            "Erro interno ao excluir incidente",
        )
...
async def get_usage(
    ...
    if not incidentId:
        return service_error_response(
            service_failure("ID do incidente é obrigatório.", 400), "Erro interno"
        )
...
async def create_image(
    ...
    if not isinstance(image, str) or not isinstance(filename, str):
        return service_error_response(
            service_failure("Dados inválidos", 400), "Erro ao salvar imagem"
        )
    ...
    if isinstance(stored, dict):
        return service_error_response(
            service_failure(stored["error"], 400), "Erro ao salvar imagem"
        )
...
async def delete_image(
    ...
    if not filename:
        return service_error_response(
            service_failure("Nome de arquivo inválido", 400), "Erro ao excluir imagem"
        )
...
def _create_incident(db: Connection, payload: dict[str, object]) -> dict[str, object]:
    ...
    if not name or len(name.strip()) < 2:
        return service_failure(
            "Nome do incidente é obrigatório e deve ter pelo menos 2 caracteres.", 400
        )
...
def _delete_incident(db: Connection, incident_id: str) -> dict[str, object]:
    ...
    if bool(category.is_system):
        return service_failure(
            f'"{category.name}" é uma categoria do sistema e não pode ser excluída.', 400
        )
    ...
    if total_usage > 0:
        if total_usage == 1:
            return service_failure(
                "Este incidente está sendo usado em 1 registro e não pode ser excluído.", 400
            )
        return service_failure(
