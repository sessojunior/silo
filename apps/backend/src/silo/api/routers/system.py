...
OLLAMA_WARMUP_TIMEOUT_SECONDS = 60.0
WARMUP_PROMPT = "oi"


class OllamaWarmupClient(Protocol):
    async def warmup(self, *, base_url: str, model: str, timeout_seconds: float) -> None: ...


class HttpxOllamaWarmupClient:
    async def warmup(self, *, base_url: str, model: str, timeout_seconds: float) -> None:
...
async def warmup_model() -> JSONResponse:
    payload, status_code = await warmup_ollama_model(settings=load_settings(), clock=SYSTEM_CLOCK)
...
async def warmup_ollama_model(
    *,
    ...
    client: OllamaWarmupClient | None = None,
