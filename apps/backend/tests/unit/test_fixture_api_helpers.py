...
def test_fixture_api_helpers_cover_validation_and_formatting_branches(monkeypatch) -> None:
    ...
    error = _payload(e2e_api._error("Falha", status=418, field="field", data={"count": 1}))
    assert error["error"] == "Falha"
    assert error["field"] == "field"
    assert error["data"] == {"count": 1}

    assert e2e_api._normalize_api_path("/api/users") == "/api/admin/users"
    assert e2e_api._normalize_api_path("/api/admin/users") == "/api/admin/users"
    assert e2e_api._normalize_api_path("/api/other") == "/api/other"

    assert e2e_api._is_safe_filename("report.pdf") is True
    assert e2e_api._is_safe_filename("") is False
    assert e2e_api._is_safe_filename("../report.pdf") is False
    assert e2e_api._is_safe_filename("folder/report.pdf") is False
    assert e2e_api._is_safe_filename("folder:report.pdf") is False
    assert e2e_api._is_safe_filename(" report.pdf") is False

    assert e2e_api._record_to_cookie_header("token-1").startswith("silo_session=token-1;")
    assert e2e_api._clear_cookie_header().endswith("Max-Age=0")

    assert e2e_api._as_list([1, 2]) == [1, 2]
    assert e2e_api._as_list("nope") == []

    assert e2e_api._content_type_for_filename("report.pdf") == "application/pdf"
    assert e2e_api._content_type_for_filename("figure.png") == "image/png"
    assert e2e_api._content_type_for_filename("photo.jpg") == "image/jpeg"
    assert e2e_api._content_type_for_filename("photo.jpeg") == "image/jpeg"
    assert e2e_api._content_type_for_filename("image.webp") == "image/webp"
    assert e2e_api._content_type_for_filename("image.gif") == "image/gif"
    assert e2e_api._content_type_for_filename("archive.bin") == "application/octet-stream"

    assert e2e_api._strip_lucide_icon_class(None) == "users"
    assert e2e_api._strip_lucide_icon_class("icon-[lucide--server]") == "server"
    assert e2e_api._strip_lucide_icon_class("shield") == "shield"
    ...
    group_count = e2e_api._group_user_count(
    ...
    chat_message = e2e_api.ChatMessageRecord(
        id="chat-1",
        content="Mensagem",
    ...
    thread = e2e_api.AssistantThreadRecord(
        id="thread-1",
    ...
    assistant_message = e2e_api.AssistantMessageRecord(
    ...
    pdf_asset = fixture_state._build_pdf_asset("!!!", ["Alpha"])
...
def test_fixture_api_state_and_assistant_branches(monkeypatch) -> None:
    ...
    assert fixture_state._thread_title("") == "Nova conversa"
    assert fixture_state._thread_title("Resumo curto") == "Resumo curto"
    assert fixture_state._thread_title(" ".join(["palavra"] * 20)).endswith("...")

    thread_summary = fixture_state._thread_summary_from_question(
        "thread-question", "Pergunta longa"
    )
    assert thread_summary.id == "thread-question"
    assert thread_summary.message_count == 2

    now = datetime(2026, 8, 4, 12, 0, tzinfo=UTC)
    old_thread = e2e_api.AssistantThreadRecord(
        id="thread-old",
        user_id=admin.id,
        title="Antigo",
    ...
    new_thread = e2e_api.AssistantThreadRecord(
        id="thread-new",
        user_id=admin.id,
        title="Novo",
    ...
    other_thread = e2e_api.AssistantThreadRecord(
        id="thread-other",
        user_id=viewer.id,
        title="Outro",
    ...
    fixture_state.assistant_messages = {
        old_thread.id: [
            e2e_api.AssistantMessageRecord(
                id="message-1",
    ...
    assert (
        fixture_state.assistant_response_for_prompt("forçar falha de artefato", old_thread.id)[
            "generation"
        ]["status"]
        == "fallback"
    )
    assert (
        fixture_state.assistant_response_for_prompt("mostre um mermaid", old_thread.id)[
            "visualization"
        ]["kind"]
        == "mermaid"
    )
    assert (
        fixture_state.assistant_response_for_prompt("quero uma image-resumo", old_thread.id)[
            "visualization"
        ]["kind"]
        == "image"
    )
    assert (
        fixture_state.assistant_response_for_prompt("quero chart e pdf", old_thread.id)[
            "artifacts"
        ][0]["kind"]
        == "pdf"
    )
    ...
    assert (
        fixture_state.assistant_response_for_prompt("faça um chart", old_thread.id)[
            "visualization"
        ]["kind"]
        == "chart"
    )
    assert (
        fixture_state.assistant_response_for_prompt("resumo textual", old_thread.id)["scope"]
        == "general"
    )

    appended_response = fixture_state.append_assistant_exchange(
        "resumo textual",
        old_thread.id,
    ...
    fixture_state.append_assistant_exchange(
        "resumo textual",
        old_thread.id,
        fixture_state.assistant_response_for_prompt("resumo textual", old_thread.id),
    )
    ...
    stream_thread = e2e_api.AssistantThreadRecord(
        id="thread-stream",
        user_id=admin.id,
        title="Stream",
    ...
    first_stream = fixture_state.build_assistant_stream_payload(
        "quero chart", stream_thread.id, "cache-key"
    )
    second_stream = fixture_state.build_assistant_stream_payload(
        "quero chart", stream_thread.id, "cache-key"
    )
    ...
    assert (
        fixture_state.get_projects_payload(project_id=e2e_api.PROJECT_ID)[0]["id"]
        == e2e_api.PROJECT_ID
    )
    ...
    assert (
        next(user for user in fixture_state.chat_users if user["id"] == admin.id)["presenceStatus"]
        == "hidden"
    )
...
def test_fixture_api_request_guards_chat_sidebar_and_uploads(monkeypatch) -> None:
    ...
    token = fixture_state.create_session(admin)
    current_user = e2e_api._current_user_from_request(_DummyRequest({"silo_session": token}))
    assert current_user is admin

    monkeypatch.setattr(e2e_api, "_current_user_from_request", lambda request: None)
    response = e2e_api._require_session(_DummyRequest())
    assert isinstance(response, JSONResponse)
    assert response.status_code == 401

    monkeypatch.setattr(e2e_api, "_current_user_from_request", lambda request: viewer)
    assert e2e_api._require_session(_DummyRequest()) is viewer

    monkeypatch.setattr(
        e2e_api,
        "_require_session",
        lambda request: e2e_api._error("Usuário não autenticado.", status=401),
    )
    response = e2e_api._require_admin(_DummyRequest())
    assert isinstance(response, JSONResponse)
    assert response.status_code == 401

    monkeypatch.setattr(e2e_api, "_require_session", lambda request: viewer)
    response = e2e_api._require_admin(_DummyRequest())
    assert isinstance(response, JSONResponse)
    assert response.status_code == 403

    monkeypatch.setattr(e2e_api, "_require_session", lambda request: admin)
    assert e2e_api._require_admin(_DummyRequest()) is admin

    base = datetime(2026, 8, 4, 12, 0, tzinfo=UTC)
    chat_messages = [
        e2e_api.ChatMessageRecord(
            id="chat-a",
            content="Antiga",
            sender_user_id=admin.id,
            sender_name="Admin",
    ...
    filtered = e2e_api._filter_chat_messages(
        chat_messages,
    ...
    sidebar = e2e_api._build_chat_sidebar_payload(admin)
    assert sidebar["totalUnread"] == 0
    assert sidebar["groups"]
    assert sidebar["users"]

    normal_pdf = e2e_api._build_upload_response("manual", "report.pdf")
    legacy_pdf = e2e_api._build_upload_response("manual", "ai-node-legacy.pdf")
    avatar_image = e2e_api._build_upload_response("avatars", "profile.png")
