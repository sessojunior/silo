...
def test_worker_main_parse_args_handles_topic_and_validation() -> None:
    args = worker_main._parse_args(["model.status", "--validation"])
...
@pytest.mark.asyncio
async def test_worker_run_uses_validation_settings_and_runs_consumer(
    ...
    result = await worker_main._run(["model.status", "--validation"])
...
def test_worker_main_main_delegates_to_asyncio_run(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        worker_main.logging, "basicConfig", lambda **kwargs: recorded.update(kwargs)
    )
