...
@pytest.mark.asyncio
async def test_assistant_examples_runtime_status_and_context_setup(
    ...
    monkeypatch.setattr(
        assistant_service,
        "load_settings",
        lambda: (_ for _ in ()).throw(RuntimeError("config ausente")),
    )
    ...
    monkeypatch.setattr(
        assistant_service,
        "load_settings",
        lambda: SimpleNamespace(vllm=SimpleNamespace(url="http://localhost:8000/v1")),
    )
    ...
    with engine.connect() as connection:
        ...
        assert state["deadline_epoch_ms"] == 1_723_000_000_000 + (
            assistant_service.ASSISTANT_GRAPH_DEADLINE_SECONDS * 1000
        )
...
def test_assistant_message_and_thread_row_helpers() -> None:
    ...
    assert assistant_service._safe_model_validate_generation({"generation": generation}) is not None
    assert (
        assistant_service._safe_model_validate_generation({"generation": {"provider": "vllm"}})
        is None
    )
    assert (
        assistant_service._safe_model_validate_visualization({"visualization": visualization})
        is not None
    )
    assert (
        assistant_service._safe_model_validate_visualization({"visualization": {"kind": "unknown"}})
        is None
    )
    assert assistant_service._safe_model_validate_artifacts({"artifacts": artifacts}) is not None
    assert assistant_service._safe_model_validate_artifacts({"artifacts": [123]}) is None
    assert (
        assistant_service._safe_model_validate_artifacts({"artifacts": [{"kind": "pdf"}]}) is None
    )
    assert assistant_service._safe_nested_value({"thinking": "ok"}, "thinking") == "ok"
    assert assistant_service._safe_nested_value("bad", "thinking") is None
    assert assistant_service._optional_text("  texto  ") == "texto"
    assert assistant_service._optional_text("   ") is None
...
def test_assistant_observability_and_plan_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        assistant_service._build_context_summary(state)
        == "scope=reports; range=2026-08-01..2026-08-04; sources="
    )
    assert assistant_service._sanitize_observability_events(None) == []
    assert assistant_service._sanitize_observability_events(
        [{"name": "node", "durationMs": 1, "status": "success"}, "bad"]
    ...
    assert assistant_service._canonical_trajectory({"refusal_reason": "fora de escopo"}) == [
        "normalize_question",
        "classify_scope",
        "refuse_out_of_scope",
        "verify_response",
    ]
    assert assistant_service._trajectory_from_results(
    ...
    assert (
        assistant_service._presentation_phase_from_state({"artifact_intent": {"kind": "mermaid"}})
        == "build_mermaid_diagram"
    )
    assert assistant_service._presentation_phase_from_state({"artifact_intent": {}}) is None
    assert assistant_service._dedupe_preserve_order(["a", "b", "a", "c"]) == ["a", "b", "c"]

    monkeypatch.setattr(assistant_service, "_current_epoch_ms", lambda: 1_723_000_100_000)
    assert assistant_service._graph_budget_exhausted({"remaining_steps": 0}) is True
    assert (
        assistant_service._graph_budget_exhausted({"deadline_epoch_ms": 1_722_999_999_999}) is True
    )
    budget_state = {"errors": []}
    assert assistant_service._graph_budget_guard(budget_state, note="classify_scope") is False
    assert budget_state["errors"] == []

    exhausted_state = {"remaining_steps": 0, "errors": []}
    assert assistant_service._graph_budget_guard(exhausted_state, note="classify_scope") is True
...
def test_assistant_date_range_helpers(
    question: str, expected: dict[str, str], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(assistant_service, "SYSTEM_CLOCK", _FixedClock())
    assert assistant_service._detect_date_range(question) == expected
...
def test_assistant_report_selection(scope: str, question: str, expected: str) -> None:
    assert assistant_service._select_report_type(question, scope) == expected
...
@pytest.mark.parametrize(
    ...
    [
        (
            "availability",
            {"availabilityReport": {"kind": "availability"}},
            {"kind": "availability"},
        ),
...
def test_assistant_pdf_selection_and_infer_helpers(
    report_type: str, results: dict[str, object], expected: dict[str, object]
) -> None:
    assert (
        assistant_service._select_report_data_for_pdf(report_type, results, {"artifact_intent": {}})
        == expected
    )
    assert assistant_service._infer_row_count({"items": [1, 2]}) == 2
    assert assistant_service._infer_row_count([1, 2, 3]) == 3
    assert assistant_service._infer_row_count({"missing": []}) is None
...
async def test_assistant_prompt_and_visualization_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert assistant_service._parse_structured_synthesis_response("") is None
    assert assistant_service._parse_structured_synthesis_response(
        '```json\n{"answer":"ok"}\n```'
    ) == {"answer": "ok"}
    assert assistant_service._parse_structured_synthesis_response("sem json") is None
    # Respostas degeneradas (curtas ou sem texto) sao recusadas.
    assert assistant_service._synthesis_answer_is_safe("0", "os modelos tiveram 0 rodadas") is False
    assert (
        assistant_service._synthesis_answer_is_safe(
            "12345678901234567890", "os modelos tiveram 0 rodadas"
        )
        is False
    )
    ...
    chart = assistant_service._build_chart_visualization(
        "projects", state, {"projectsSnapshot": {"projects": [{"name": "P1"}]}}
    )
    image = assistant_service._build_image_visualization(
        "projects", state, {"projectsSnapshot": {"totalProjects": 2}}
    )
    mermaid = assistant_service._build_mermaid_visualization(
        "projects", state, {"projectsSnapshot": {"projects": []}}
    )
...
async def test_assistant_grounded_text_helpers_cover_all_scope_formats() -> None:
    ...
    for scope, results, expected_snippet, expected_secondary in cases:
        state = {
            ...
            "artifact_result": {
                ...
                "url": "/reports/projects.pdf",
            }
            if scope == "generate_pdf"
            else {},
        }
        answer, citations, suggestions = assistant_service._build_grounded_text(
    ...
    pdf_answer, pdf_citations, pdf_suggestions = assistant_service._build_grounded_text(
        pdf_state, runtime_context
    )
...
def test_assistant_history_cache_and_persisted_response_helpers(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
    metadata = MetaData()
    ...
    with engine.connect() as connection:
        ...
        clarification_pdf = assistant_service._build_clarification_from_entities(
            {
                "problemCategories": {
                    "matches": [{"id": "a", "name": "A"}, {"id": "b", "name": "B"}]
                }
            },
            "generate_pdf",
        )
        ...
        assert (
            assistant_service._build_clarification_from_entities(
        ...
        cache_key = assistant_service._semantic_cache_key(
            {"question": "Resumo de projetos"}, runtime_context, plan
        )
        ...
        assert assistant_service._artifact_row(
            {"artifact_result": {"artifact": {"kind": "pdf"}}}
        ) == {"kind": "pdf"}
        assert assistant_service._artifact_row({"artifact_result": {"status": "ok"}}) is None

        monkeypatch.setattr(
            assistant_service, "_current_thread_summary", lambda *args, **kwargs: None
        )
        monkeypatch.setattr(
            assistant_service,
            "load_settings",
        ...
        cache_runtime = assistant_service._build_runtime_context_from_cache(
            connection, {"request_id": "req-1", "run_id": "run-1"}
        )
...
def test_assistant_thread_crud_and_state_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with engine.connect() as connection:
        ...
        loaded = assistant_service._load_thread_row_by_id(connection, "thread-1")
        assert loaded is not None and loaded["user_id"] == "user-1"
        assert assistant_service._load_thread_row_by_id(connection, "missing") is None
        ...
        tied_details = assistant_service.get_assistant_thread_details(
            connection, "user-1", "thread-tie"
        )
        ...
        assert [message.id for message in details.messages] == [
            "message-1",
            "message-2",
            "message-3",
        ]
        ...
        assert (
            assistant_service.get_assistant_thread_details(connection, "user-2", "thread-1") is None
        )

        summary = assistant_service._current_thread_summary(connection, "user-1", "thread-1")
        assert summary is not None and summary.id == "thread-1"
        fallback_summary = assistant_service._current_thread_summary(
            connection, "user-2", "thread-1"
        )
        assert fallback_summary is not None and fallback_summary.id == "thread-1"

        visualization = assistant_service._current_visualization(
            {
                "visualization": {
                    "kind": "chart",
                    "chartType": "bar",
        ...
        assert assistant_service._current_visualization({"visualization": "bad"}) is None

        artifacts = assistant_service._current_artifacts(
            {
                "artifact_result": {
                    "status": "claimed",
                    "artifact": {
        ...
        assert (
            assistant_service._current_artifacts({"artifact_result": {"status": "ready"}}) is None
        )

        assert (
            assistant_service._thread_title_from_question("   " + ("A" * 80) + " ")
            == ("A" * 57) + "..."
        )
        assert assistant_service._thread_title_from_question("   ") == "Nova conversa"
        assert assistant_service._hash_json({"b": 2, "a": 1}) == assistant_service._hash_json(
            {"a": 1, "b": 2}
        )

        assistant_service._delete_artifact_file_if_present(
            {"kind": "image", "filename": "ignored.webp"}
        )
        assistant_service._delete_artifact_file_if_present(
            {"kind": "pdf", "filename": "report.pdf"}
        )
        assistant_service._delete_artifact_file_if_present({"kind": "pdf", "filename": None})
        assert delete_calls == [("reports", "report.pdf")]

        delete_calls.clear()
        assistant_service._recalculate_thread_state(connection, "thread-empty")
        refreshed_empty = assistant_service._load_thread_row_by_id(connection, "thread-empty")
        ...
        assistant_service._prune_thread_messages(connection, "thread-prune")
        ...
        with pytest.raises(assistant_service.AssistantThreadNotFoundError):
            assistant_service.delete_assistant_message(
                connection, "user-2", "thread-1", "message-1"
            )
        with pytest.raises(assistant_service.AssistantThreadNotFoundError):
            assistant_service.delete_assistant_message(
                connection, "user-1", "thread-1", "missing-message"
            )

        assistant_service.delete_assistant_message(connection, "user-1", "thread-1", "message-2")
        assert delete_calls == [("reports", "second.pdf")]
        updated_thread = assistant_service._load_thread_row_by_id(connection, "thread-1")
        ...
        assert assistant_service._load_thread_row_by_id(connection, "thread-1") is None
...
def test_assistant_persistence_helpers_cover_message_and_pdf_artifact_writes(
    ...
    monkeypatch.setattr(
        ...
        lambda kind, filename: f"/uploads/{kind}/{filename}",
    )

    class FakeArtifactRepository:
        instances: ClassVar[list[FakeArtifactRepository]] = []
    ...
    with engine.connect() as connection:
        ...
        state = {
            ...
            "final_response": {
                ...
                "visualization": {
                    "kind": "image",
                    "src": "https://example.test/preview.webp",
                    "alt": "preview",
                },
                "artifacts": [
                    {
        ...
        assistant_service._persist_user_and_assistant_messages(runtime_context, state)
        thread_row = assistant_service._load_thread_row_by_id(connection, "thread-persist")
        ...
        assistant_service._finalize_pdf_artifact(runtime_context, state)
...
def test_assistant_validation_and_thread_lookup_branches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert assistant_service._build_runtime_context_from_state({}, runtime) is runtime.context
    assert assistant_service._safe_model_validate_generation("bad") is None
    assert assistant_service._safe_model_validate_visualization("bad") is None
    assert assistant_service._safe_model_validate_artifacts("bad") is None
    assert assistant_service._safe_model_validate_artifacts({"artifacts": []}) is None

    valid_artifacts = assistant_service._safe_model_validate_artifacts(
        {
            "artifacts": [
    ...
    assert assistant_service._validate_visualization_payload(image_payload).kind == "image"
    assert assistant_service._validate_visualization_payload(mermaid_payload).kind == "mermaid"
    with pytest.raises(ValueError):
        assistant_service._validate_visualization_payload({"kind": "unknown"})

    with engine.connect() as connection:
        current_user = CurrentUser(
            id="user-1", email="user@example.test", name="User One", is_active=True
        )
        existing = assistant_service._get_thread_or_create(connection, current_user, "thread-1")
        assert existing["id"] == "thread-1"
        assert assistant_service._load_thread_row(connection, "user-1", "thread-1") is not None
        assert assistant_service._load_thread_row(connection, "user-2", "thread-1") is None

        with pytest.raises(assistant_service.AssistantThreadNotFoundError):
            assistant_service._get_thread_or_create(connection, current_user, "missing-thread")

        created = assistant_service._get_thread_or_create(connection, current_user, None)
        assert created["user_id"] == "user-1"
        assert assistant_service._load_thread_row_by_id(connection, created["id"]) is not None


@pytest.mark.asyncio
async def test_assistant_scope_detection_and_resolution_branches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _EmbeddingProvider:
        ...
        async def embed(self, text: str) -> tuple[float, float]:
            models_profile = (
                "disponibilidade de modelos, rodadas, intervenções e execução operacional"
            )
    ...
    assert assistant_service._scope_candidates("Modelos e disponibilidade")[:2][0][0] == "models"

    detected_scope, confidence = await assistant_service._detect_scope(
        "Modelos e disponibilidade", runtime_context
    )
    assert detected_scope == "models"
    assert confidence > 0.7

    embedding_scope = await assistant_service._detect_scope_by_embedding(
        "Modelos recentes", runtime_context
    )
    assert embedding_scope == "models"

    embedding_none = await assistant_service._detect_scope_by_embedding(
        "Modelos recentes",
        SimpleNamespace(
            connection=connection,
    ...
    model_scope = await assistant_service._detect_scope_by_model(
    ...
    assert assistant_service._select_report_type("Gerar PDF", "generate_pdf") == "executive"
    assert assistant_service._select_report_type("Atualização", "unknown") == "executive"
    assert assistant_service._required_sources_for_scope("generate_pdf") == ("report_pdf",)
    assert assistant_service._required_sources_for_scope("desconhecido") == ("executive_report",)

    monkeypatch.setattr(
        assistant_service, "resolve_models", lambda _connection, _question: ["model-1"]
    )
    monkeypatch.setattr(
        assistant_service, "resolve_projects", lambda _connection, _question: ["project-1"]
    )
    monkeypatch.setattr(
        assistant_service,
        "resolve_problem_categories",
        lambda _connection, _question: ["category-1"],
    )

    assert assistant_service._resolve_entities("sobre modelos", "models", connection) == {
        "models": ["model-1"]
    }
    assert assistant_service._resolve_entities("sobre projetos", "projects", connection) == {
        "projects": ["project-1"]
    }
    assert assistant_service._resolve_entities("sobre problemas", "problems", connection) == {
        "problemCategories": ["category-1"]
    }
    assert assistant_service._resolve_entities("geral", "general", connection) == {}


@pytest.mark.asyncio
async def test_assistant_scope_plan_and_pdf_claim_fallback_branches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
    connection = engine.connect()
    current_user = CurrentUser(id="user-1", email="user@example.test", name="User", is_active=True)
    original_detect_scope_by_embedding = assistant_service._detect_scope_by_embedding
    original_detect_scope_by_model = assistant_service._detect_scope_by_model
    runtime_context = SimpleNamespace(
        connection=connection,
    ...
    async def _unexpected_scope_call(*args, **kwargs):
    ...
    detected_scope, detected_confidence = await assistant_service._detect_scope(
        "Modelos e disponibilidade", runtime_context
    )
    assert detected_scope == "models"
    ...
    monkeypatch.setattr(assistant_service, "_detect_scope_by_model", _unexpected_scope_call)
    detected_scope, detected_confidence = await assistant_service._detect_scope(
        "Resumo ambíguo", runtime_context
    )
    ...
    async def _model_scope_call(
        question: str, runtime_context: object, keyword_scores: list[tuple[str, float]]
    ) -> str | None:
    ...
    monkeypatch.setattr(assistant_service, "_detect_scope_by_model", _unexpected_scope_call)
    detected_scope, detected_confidence = await assistant_service._detect_scope(
        "Pergunta aberta", runtime_context
    )
    ...
    monkeypatch.setattr(
        assistant_service, "_detect_scope_by_embedding", original_detect_scope_by_embedding
    )
    embedding_scope = await assistant_service._detect_scope_by_embedding(
        "Modelos recentes",
    ...
    monkeypatch.setattr(
        assistant_service,
        "_detect_date_range",
        lambda question: {"start": "2026-08-01", "end": "2026-08-04"},
    )
    monkeypatch.setattr(
        assistant_service, "_select_report_type", lambda question, scope: "projects"
    )
    monkeypatch.setattr(
        assistant_service,
        "_resolve_entities",
        lambda question, scope, connection: {"projects": ["project-1"]},
    )

    plan = await assistant_service._plan_from_question("comparar projetos", runtime_context, {})
    ...
    result_state = await assistant_service._node_guard_and_normalize(empty_state, runtime)
    ...
    result_state = await assistant_service._node_guard_and_normalize(long_state, runtime)
    assert result_state["refusal_reason"] == "A pergunta excede o limite permitido."
    assert result_state["final_response"]["refusal"] is True

    monkeypatch.setattr(
        assistant_service,
        "_get_thread_or_create",
        lambda connection, current_user, thread_id: {"id": "thread-1"},
    )
    monkeypatch.setattr(
        assistant_service,
        "_load_recent_history",
        lambda connection, thread_id: [{"scope": "models"}],
    )
    monkeypatch.setattr(
        assistant_service, "_build_conversation_memory", lambda history_messages: "memory"
    )
    ...
    result_state = await assistant_service._node_guard_and_normalize(valid_state, runtime)
    ...
    result_state = await assistant_service._node_classify_and_plan(plan_state, runtime)
    ...
    async def _plan_stub(
        question: str, runtime_context: object, state: object
    ) -> assistant_service.AssistantPlan:
        del question, runtime_context, state
        return plan_stub

    monkeypatch.setattr(assistant_service, "_plan_from_question", _plan_stub)
    monkeypatch.setattr(
        assistant_service, "_semantic_cache_key", lambda state, runtime_context, plan: "cache-key"
    )
    ...
    monkeypatch.setattr(
        assistant_service, "AiArtifactRepository", lambda connection: _Repo({"status": "ignored"})
    )
    result_state = await assistant_service._node_claim_pdf_idempotency_if_needed(
        non_pdf_state, runtime
    )
    ...
    monkeypatch.setattr(
        ...
        lambda connection: _Repo(
            {"status": assistant_service.AI_ARTIFACT_READY, "id": "artifact-1"}
        ),
    )
    monkeypatch.setattr(
        assistant_service,
        "_load_persisted_response_from_artifact",
        lambda connection, artifact_row, state: {"loaded": True, "artifact": artifact_row},
    )
    result_state = await assistant_service._node_claim_pdf_idempotency_if_needed(
        ready_state, runtime
    )
    ...
    assert result_state["final_response"] == {
        "loaded": True,
        "artifact": {"status": assistant_service.AI_ARTIFACT_READY, "id": "artifact-1"},
    }
    ...
    monkeypatch.setattr(
        ...
        "AiArtifactRepository",
        lambda connection: _Repo(
            {"status": assistant_service.AI_ARTIFACT_PENDING, "id": "artifact-2"}
        ),
    )
    result_state = await assistant_service._node_claim_pdf_idempotency_if_needed(
        pending_state, runtime
    )
    ...
    monkeypatch.setattr(
        assistant_service,
        "AiArtifactRepository",
        lambda connection: _Repo({"status": "existing", "id": "artifact-3"}),
    )
    result_state = await assistant_service._node_claim_pdf_idempotency_if_needed(
        existing_state, runtime
    )
    ...
    monkeypatch.setattr(
        assistant_service, "AiArtifactRepository", lambda connection: _Repo(_Claim())
    )
    result_state = await assistant_service._node_claim_pdf_idempotency_if_needed(
        claimed_state, runtime
    )
...
@pytest.mark.asyncio
async def test_assistant_node_runtime_helpers_cover_remaining_branches(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    engine = create_engine("sqlite+pysqlite:///:memory:", future=True)
    connection = engine.connect()
    ...
    loaded_state = await assistant_service._node_load_persisted_result(load_state, runtime)
    assert loaded_state["final_response"]["refusal"] is True

    monkeypatch.setattr(
        assistant_service, "_artifact_row", lambda _state: {"kind": "pdf", "filename": "report.pdf"}
    )
    monkeypatch.setattr(
        assistant_service,
        "_load_persisted_response_from_artifact",
    ...
    loaded_state = await assistant_service._node_load_persisted_result({"progress": []}, runtime)
    ...
    assert (
        await assistant_service._node_semantic_cache_if_text_only(cache_state, runtime)
        is cache_state
    )
    ...
    cached_state = await assistant_service._node_semantic_cache_if_text_only(cache_state, runtime)
    ...
    await assistant_service._node_semantic_cache_if_text_only(expired_state, runtime)
    ...
    monkeypatch.setattr(
        ...
        lambda state, kind, name, duration_ms, status="success": observed_tools.append(
            (kind, name, duration_ms, status)
        ),
    )

    tool_state = {"errors": []}
    result = await assistant_service._run_required_tool(
        tool_state,
        runtime_context,
        "tool-success",
    ...
    async def _slow_to_thread(_call):
        await asyncio.sleep(0.01)

    monkeypatch.setattr(assistant_service.asyncio, "to_thread", _slow_to_thread)
    timeout_result = await assistant_service._run_required_tool(
        tool_state,
        runtime_context,
        "tool-timeout",
        lambda _connection: {"ok": True},
    ...
    async def _explode_to_thread(_call):
        raise RuntimeError("boom")

    monkeypatch.setattr(assistant_service.asyncio, "to_thread", _explode_to_thread)
    error_result = await assistant_service._run_required_tool(
        tool_state,
        runtime_context,
        "tool-error",
        lambda _connection: {"ok": True},
    ...
    async def _fake_run_required_tool(
        state, runtime_context, tool_name, callback, *, timeout_seconds
    ):
    ...
    await assistant_service._run_required_tool_batch(
        runtime_context,
        results,
        batch_state,
        [
    ...
    refused_state = await assistant_service._node_execute_required_data_tools(tool_state, runtime)
    assert refused_state["final_response"]["refusal"] is True

    runtime_context.has_reports_permission = True

    async def _boom_batch(*args, **kwargs):
        raise RuntimeError("batch boom")

    monkeypatch.setattr(assistant_service, "_run_required_tool_batch", _boom_batch)
    errored_state = await assistant_service._node_execute_required_data_tools(
        {"progress": [], "errors": [], "scope": "reports", "question": "Resumo"}, runtime
    )
    ...
    class _FakeRegistry:
        ...
        def register(self, name, data, *, schema_id, source_kind, row_count, clock):
            self.calls.append(
                {
                    "name": name,
                    "schema_id": schema_id,
    ...
    monkeypatch.setattr(
        assistant_service,
        "_build_grounded_text",
        lambda state, context: ("Resposta", [], ["Pergunta 1"]),
    )
    monkeypatch.setattr(
        assistant_service,
        "_combined_tool_results",
        lambda state: {"projectsReport": {"items": [1, 2]}, "availabilityReport": {"items": [1]}},
    )
    dataset_state = {
        "progress": [],
        "scope": "projects",
    ...
    dataset_state = await assistant_service._node_analyze_and_register_datasets(
        dataset_state, runtime
    )
    ...
    monkeypatch.setattr(
        ...
        lambda scope, state, results: _Viz(
            {"kind": "mermaid", "scope": scope, "intent": "mermaid"}
        ),
    )

    async def _fake_build_pdf_artifact(runtime_context, state, results):
    ...
    for intent in ("chart", "image", "mermaid", "pdf"):
        presentation_state = {
            "progress": [],
            "scope": "projects",
            "artifact_intent": {"kind": intent},
            "required_results": {"projectsReport": {}},
        }
        presentation_state = await assistant_service._node_presentation_router(
            presentation_state, runtime
        )
    ...
    synth_state = await assistant_service._node_synthesize_once(synth_state, runtime)
    ...
    class _MetadataRuntime:
        async def complete_with_metadata(self, messages):
            del messages
            return (
                SimpleNamespace(
                    content='{"answer":"Resposta final completa","contextSummary":"resumo"}'
                ),
    ...
    token_limited_state = {
        "progress": [],
        "response_base": "Fallback",
        "question": "Resumo",
        "scope": "projects",
    }
    token_limited_state = await assistant_service._node_synthesize_once(
        token_limited_state, runtime
    )
    ...
    simple_state = await assistant_service._node_synthesize_once(simple_state, runtime)
    ...
    exploded_state = await assistant_service._node_synthesize_once(exploded_state, runtime)
    ...
    monkeypatch.setattr(
        ...
        lambda scope, state: [
            assistant_service.AiAssistantCitationDto(label="Fonte", detail=scope)
        ],
    )
    validate_state = await assistant_service._node_validate_output_citations_and_artifacts(
        validate_state, runtime
    )
    assert validate_state["final_response"]["citations"][0]["label"] == "Fonte"
    assert validate_state["final_response"]["refusal"] is False

    refusal_validate_state = {
        "progress": [],
        "scope": "projects",
        "refusal_reason": "fora de escopo",
    }
    refusal_validate_state = await assistant_service._node_validate_output_citations_and_artifacts(
        refusal_validate_state, runtime
    )
    ...
    monkeypatch.setattr(
        assistant_service,
        "_persist_user_and_assistant_messages",
        lambda runtime_context, state: persist_calls.append("persist"),
    )
    monkeypatch.setattr(
        assistant_service,
        "_finalize_pdf_artifact",
        lambda runtime_context, state: persist_calls.append("finalize"),
    )
    monkeypatch.setattr(
        assistant_service, "_store_semantic_cache", lambda state: persist_calls.append("cache")
    )

    attached_state = {
        "progress": [],
        "artifact_result": {"status": "attached_hit"},
        "final_response": None,
    }
    attached_state = await assistant_service._node_persist_transaction(attached_state, runtime)
    assert attached_state["final_response"] is not None
    assert persist_calls == []

    claimed_state = {
        "progress": [],
        "artifact_result": {"status": "claimed"},
        "final_response": None,
    }
    claimed_state = await assistant_service._node_persist_transaction(claimed_state, runtime)
    ...
    emit_state = await assistant_service._node_emit_result(emit_state, runtime)
    ...
    assert assistant_service._extract_tool_calls(tool_calls_message) == [
        {"id": "1", "name": "tool-a", "args": {"x": 1}}
    ]
    assert assistant_service._tool_call_signature(
        "tool-a", {"x": 1}
    ) == assistant_service._tool_call_signature("tool-a", {"x": 1})
    assert assistant_service._hybrid_result_key("search_silo_knowledge") == "knowledgeSearch"
    assert assistant_service._hybrid_result_key("unknown") == "unknown"
    assert assistant_service._parse_model_scope_response('```json\n{"scope":"projects"}\n```') == {
        "scope": "projects"
    }
    assert assistant_service._parse_model_scope_response("sem json") is None
    assert assistant_service._compact_tool_result({"value": object()})[:1] == "{"

    state_for_observed = {"progress": []}

    async def _observed_handler(state, runtime):
        state["handled"] = True
        return state

    wrapper = assistant_service._observed_node("demo", _observed_handler)
...
def test_question_is_out_of_scope_matches_eval_cases() -> None:
    def _is_out(question: str, scope: str) -> bool:
        return assistant_service._question_is_out_of_scope(
            question, scope, assistant_service._scope_candidates(question)
        )
...
@pytest.mark.asyncio
async def test_classify_and_plan_refuses_out_of_scope_questions(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    plan_stub = assistant_service.AssistantPlan(
        scope="general",
    ...
    monkeypatch.setattr(
        assistant_service, "_semantic_cache_key", lambda state, context, plan: "cache-key"
    )

    runtime = SimpleNamespace(context=SimpleNamespace())

    out_state = {"question": "Qual a previsão do tempo para amanhã?", "progress": []}
    result_state = await assistant_service._node_classify_and_plan(out_state, runtime)
    ...
    result_state = await assistant_service._node_classify_and_plan(in_state, runtime)
