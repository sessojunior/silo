...
description: "Use when changing the version shown in the SILO web sidebar or any app version metadata in apps/frontend. Keep it centralized in apps/frontend/src/lib/config.ts and do not use env vars, CI metadata, package.json, or extra abstractions."
...
- A versão vive somente em `apps/frontend/src/lib/config.ts` como `config.appVersion`.
...
1. Editar `apps/frontend/src/lib/config.ts`.
