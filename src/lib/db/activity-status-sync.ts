 ...
 */
export async function syncActivityStatus(
  activityId: string,
): Promise<"todo" | "progress" | "done" | "blocked"> {
  try {
    // Buscar todas as tarefas da atividade
    const tasks = await db
      .select()
      .from(schema.projectTask)
      .where(eq(schema.projectTask.projectActivityId, activityId));
    ...
    console.error("❌ [LIB_ACTIVITY_STATUS_SYNC] Erro ao sincronizar status:", {
      error,
    });
    throw error;
  }
 ...
 */
export async function syncAllActivitiesStatus(
  projectId: string,
): Promise<void> {
  try {
    ...
    console.error(
      "❌ [LIB_ACTIVITY_STATUS_SYNC] Erro ao sincronizar atividades:",
      { error },
    );
    throw error;
  }
