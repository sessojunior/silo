...
export type RestConsumerInstance = {
  groupId: string;
  instanceId: string;
  baseUri: string; // full URL to instance base
};
...
export async function createRestConsumer(groupId: string, instanceName?: string, offsetReset: "earliest" | "latest" = "latest") {
  const url = baseUrl();
  ...
  const instanceId = (data.instance_id || data.instanceId || name) as string;
  const base_uri = (data.base_uri || data.baseUri || `/consumers/${groupId}/instances/${instanceId}`) as string;
  const base = base_uri.startsWith("http") ? base_uri : `${url}${base_uri.startsWith("/") ? base_uri : `/${base_uri}`}`;
  return { groupId, instanceId, baseUri: base } as RestConsumerInstance;
}

export async function subscribeRest(instance: RestConsumerInstance, topics: string[]) {
...
export async function fetchRecordsRest(instance: RestConsumerInstance, timeoutMs = 10000): Promise<RestRecord[]> {
...
export async function commitOffsetsRest(instance: RestConsumerInstance, offsets: Array<{ topic: string; partition: number; offset: string }>) {
...
export async function deleteRestConsumer(instance: RestConsumerInstance) {
  const res = await fetch(instance.baseUri, { method: "DELETE", headers: buildHeaders(undefined, JSON_V2) });
  if (!res.ok) {
    const txt = await res.text();
...
export async function produceRecordRest(topic: string, value: string | object, key?: string) {
  const url = baseUrl();
