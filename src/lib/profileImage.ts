...
export async function uploadProfileImageFromUrl(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Falha ao baixar imagem. Status HTTP: ${response.status}`,
      );
    }
    ...
    if (!contentType.startsWith("image/")) {
      throw new Error(
        `Tipo de conteúdo inválido: ${contentType}. Esperado tipo 'image/*'.`,
      );
    }
    ...
    console.error(
      "❌ [LIB_PROFILE_IMAGE] Erro ao processar/salvar imagem de perfil:",
      { userId, error: err },
    );
    return false;
  }
...
export async function uploadProfileImageFromInput(
  file: File,
  userId: string,
): Promise<
...
> {
  ...
  try {
    ...
    if (file.size > IMAGE_SIZE_UPLOAD) {
      console.warn(
        "⚠️ [LIB_PROFILE_IMAGE] Arquivo excede o limite de tamanho:",
        { limit: IMAGE_SIZE_UPLOAD / (1024 * 1024) },
      );
      return {
        error: {
          code: "FILE_TOO_LARGE",
          message: `O arquivo deve ter no máximo ${IMAGE_SIZE_UPLOAD / (1024 * 1024)} MB.`,
        },
      };
    }
    ...
    if (!metadata.format || !IMAGE_ALLOWED_FORMATS.includes(metadata.format)) {
      console.warn(
        "⚠️ [LIB_PROFILE_IMAGE] Formato de imagem inválido detectado:",
    ...
    if (!metadata.width || !metadata.height) {
      console.warn(
        "⚠️ [LIB_PROFILE_IMAGE] Não foi possível obter os dados da imagem",
      );
      return {
        error: {
    ...
    console.error(
      "❌ [LIB_PROFILE_IMAGE] Erro ao processar ou salvar imagem de perfil:",
      { userId, error: err },
    );
    return {
      error: {
...
export function deleteUserProfileImage(
  ...
  try {
    ...
    console.error("❌ [LIB_PROFILE_IMAGE] Erro ao deletar imagem de perfil:", {
      error,
    });
    return {
      error: {
        code: "DELETE_ERROR",
        message: "Erro ao deletar a imagem de perfil.",
      },
    };
  }
