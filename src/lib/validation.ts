...
const projectStatusEnumValues = ['planning', 'active', 'on_hold', 'completed', 'cancelled'] as const
const projectActivityStatusEnumValues = ['pending', 'in_progress', 'completed', 'cancelled'] as const
const problemStatusEnumValues = ['open', 'in_progress', 'resolved', 'closed'] as const
...
export const problemSchema = z.object({
	...
	priority: z.enum(priorityEnumValues, {
		error: () => ({ message: 'Prioridade deve ser: low, medium, high ou critical' })
	}),
	
	status: z.enum(problemStatusEnumValues, {
		error: () => ({ message: 'Status deve ser: open, in_progress, resolved ou closed' })
	})
})
...
export const productSchema = z.object({
	...
	status: z.enum(['active', 'inactive', 'maintenance'] as const, {
		error: () => ({ message: 'Status deve ser: active, inactive ou maintenance' })
	}),
...
export const projectSchema = z.object({
	...
	status: z.enum(projectStatusEnumValues, {
		error: () => ({ message: 'Status deve ser: planning, active, on_hold, completed ou cancelled' })
	}),
...
export const projectActivitySchema = z.object({
	...
	status: z.enum(projectActivityStatusEnumValues, {
		error: () => ({ message: 'Status deve ser: pending, in_progress, completed ou cancelled' })
	}),
