...
export async function POST(request: NextRequest) {
  try {
    ...
    if (files.length > 3) {
      return errorResponse("Máximo de 3 arquivos por upload.", 400);
    }
    ...
    return successResponse(
      uploaded,
      `${uploaded.length} arquivo(s) de incidente enviado(s) com sucesso!`,
    );
  } catch (error) {
    console.error(
      "❌ [API_UPLOAD_INCIDENTS] Erro no proxy de upload de incidentes:",
      { error },
    );
