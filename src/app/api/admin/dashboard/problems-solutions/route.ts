...
export async function GET() {
  try {
    ...
    const problemsRows = await db
      ...
      .where(
        gte(
          ...
          new Date(today.getTime() - (TOTAL_DAYS - 1) * 86_400_000),
        ),
      );
    ...
    const fmt = (d: Date) =>
      formatDateBR(d.toISOString().split("T")[0]).replace(/\d{4}/, "").trim();
    ...
    function getDayIndex(dateObj: Date): number | null {
      const diffDays = Math.floor(
        (dateObj.getTime() -
          (today.getTime() - (TOTAL_DAYS - 1) * 86_400_000)) /
          86_400_000,
      ); // Corrigido: cálculo ajustado
    ...
    return successResponse({
      categories,
      problems: problemsCounts,
      solutions: solutionsCounts,
    });
  } catch (error) {
    console.error(
      "❌ [API_DASHBOARD_PROBLEMS_SOLUTIONS] Erro ao obter problemas & soluções:",
      { error },
    );
    return errorResponse("Erro ao obter dados de problemas & soluções", 500);
  }
}
