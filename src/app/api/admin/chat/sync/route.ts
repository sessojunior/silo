...
export async function GET(request: NextRequest) {
  try {
    ...
    const sinceDate = sinceTimestamp
      ...
      : new Date(Date.now() - 5 * 60 * 1000); // 5 minutos atrás por padrão
    ...
    const filteredPresence = updatedPresence.filter(
      (p) => p.updatedAt > fiveSecondsAgo,
    );
    ...
    for (const msgCount of userMessageCounts) {
      const unreadByUser = await db
        ...
        .where(
          and(
            eq(schema.chatMessage.senderUserId, msgCount.senderUserId),
            eq(schema.chatMessage.receiverUserId, user.id),
    ...
    const messagesWithType = newMessages.map((msg) => ({
      ...msg,
      messageType: msg.receiverGroupId
        ? ("groupMessage" as const)
