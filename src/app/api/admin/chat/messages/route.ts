...
async function getTotalMessagesCount(
  ...
  try {
        ...
        .where(
          and(
            or(
              ...
              and(
                eq(schema.chatMessage.senderUserId, currentUserId),
              ...
              and(
                ...
                eq(schema.chatMessage.receiverUserId, currentUserId),
              ),
            ),
            isNull(schema.chatMessage.deletedAt),
          ),
    ...
    console.error("❌ [API_CHAT_MESSAGES] Erro ao contar mensagens totais:", {
      error,
    });
    return 0;
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (groupId) {
      ...
      if (before) {
        whereConditions.push(
          lt(schema.chatMessage.createdAt, new Date(before)),
        );
      }
    ...
    console.error("❌ [API_CHAT_MESSAGES] Erro ao buscar mensagens:", {
      error,
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!content || content.trim().length === 0) {
      return errorResponse("Conteúdo da mensagem é obrigatório", 400);
    }
    ...
    // Apenas um receptor
    if (
      (receiverGroupId && receiverUserId) ||
      (!receiverGroupId && !receiverUserId)
    ) {
      return errorResponse(
    ...
    if (receiverUserId) {
      ...
      const targetUser = await db
        ...
        .where(eq(schema.authUser.id, receiverUserId))
        .limit(1);

      if (targetUser.length === 0) {
        return errorResponse("Usuário destinatário não encontrado", 404);
      }
    }
