  ...
  try {
    ...
    const tasksWithUsersData = await db
      ...
      .leftJoin(
        ...
        eq(schema.projectTask.id, schema.projectTaskUser.taskId),
      )
      .leftJoin(
        schema.authUser,
      ...
      .where(
        and(
          ...
          eq(schema.projectTask.projectActivityId, activityId),
        ),
      )
    ...
    return successResponse({ tasks: groupedTasks });
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    function tasksArrayEquals(
      a: { taskId: string; status: string; sort: number }[],
      b: { taskId: string; status: string; sort: number }[],
    ) {
      if (a.length !== b.length) return false;
      const sortFn = (x: { taskId: string }, y: { taskId: string }) =>
        x.taskId.localeCompare(y.taskId);
      const arrA = [...a].sort(sortFn);
      const arrB = [...b].sort(sortFn);
      for (let i = 0; i < arrA.length; i++) {
        if (
          arrA[i].taskId !== arrB[i].taskId ||
          arrA[i].status !== arrB[i].status ||
          arrA[i].sort !== arrB[i].sort
        ) {
          return false;
        }
      }
    ...
    if (!isEqual) {
      ...
      return errorResponse("KANBAN_OUTDATED", 409, { tasks: dbTasks });
    }

    // Identificar mudanças para histórico
    const historyEntries = [];
    for (const taskAfter of tasksAfterMove) {
      const taskBefore = tasksBeforeMove.find(
        (t) => t.taskId === taskAfter.taskId,
      );
      ...
      if (
        ...
        (taskBefore.status !== taskAfter.status ||
          taskBefore.sort !== taskAfter.sort)
      ) {
        historyEntries.push({
    ...
    await db.transaction(async (tx) => {
      ...
      for (const task of tasksAfterMove) {
        await tx
          ...
          .set({ status: task.status, sort: task.sort, updatedAt: new Date() })
          .where(eq(schema.projectTask.id, task.taskId));
      }
    });
    ...
    return successResponse({ tasks: tasksWithUsers });
  } catch (error) {
    console.error(
      "❌ [API_PROJECTS_TASKS] Erro ao atualizar tarefas do kanban:",
      { error },
    );
    return errorResponse("Erro ao atualizar tarefas do kanban", 500);
  }
}
...
export async function POST(
  ...
  try {
    ...
    const existingTasks = await db
      ...
      .where(
        and(
          eq(schema.projectTask.projectId, projectId),
          eq(schema.projectTask.projectActivityId, activityId),
          eq(schema.projectTask.status, status),
        ),
      );
    ...
    return successResponse({ task: newTask[0] });
  } catch (error) {
...
export async function PUT(
  ...
  try {
    ...
    return successResponse({ task: updatedTask[0] });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    return successResponse(null, "Tarefa excluída com sucesso");
  } catch (error) {
