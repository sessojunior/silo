...
export async function GET() {
  ...
  try {
    ...
    console.error("❌ [API_MANUAL_IMAGES] Erro ao listar imagens do manual:", {
      error,
    });
...
export async function DELETE(req: NextRequest) {
  const authResult = await requirePermissionAuthUser("productManual", "update");
  if (!authResult.ok) return authResult.response;
  ...
  try {
    ...
    console.error("❌ [API_MANUAL_IMAGES] Erro ao excluir imagem do manual:", {
      error,
    });
