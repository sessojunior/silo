...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const users = userIds.length
      ? await db.select().from(authUser).where(inArray(authUser.id, userIds))
      : [];
    ...
    console.error("❌ [API_PRODUCTS_SOLUTIONS] Erro ao buscar soluções:", {
      error,
    });
    return errorResponse("Erro ao buscar soluções.", 500);
  }
}
...
export async function POST(req: NextRequest) {
  ...
  if (!problemId || description.length < 2) {
    return errorResponse(
      "Descrição e problema são obrigatórios (mín. 2 caracteres).",
      400,
    );
  }
  ...
  return successResponse(null, "Solução criada com sucesso", 201);
}
...
export async function PUT(req: NextRequest) {
  ...
  if (!id || description.length < 2) {
    return errorResponse(
      "ID e descrição são obrigatórios (mín. 2 caracteres).",
      400,
    );
  }
  ...
  const solution = await db
    .select()
    .from(productSolution)
    .where(eq(productSolution.id, id));
  ...
  if (imageUrl) {
    ...
    const oldImg = await db
      .select()
      .from(productSolutionImage)
      .where(eq(productSolutionImage.productSolutionId, id));
    if (oldImg.length) {
      await db
  ...
  return successResponse(null, "Solução atualizada com sucesso");
}
  ...
  await db.transaction(async (tx) => {
    ...
    const getAllChildReplies = async (parentId: string): Promise<string[]> => {
      const directReplies = await tx
        .select()
        .from(productSolution)
        .where(eq(productSolution.replyId, parentId));
    ...
    if (allSolutionIds.length > 0) {
      await tx
        ...
        .where(
          inArray(productSolutionChecked.productSolutionId, allSolutionIds),
        );
    }
    ...
    if (allSolutionIds.length > 0) {
      await tx
        .delete(productSolutionImage)
        .where(inArray(productSolutionImage.productSolutionId, allSolutionIds));
    }

    // 5. Excluir todas as soluções (principal + respostas filhas)
    await tx
      .delete(productSolution)
      .where(inArray(productSolution.id, allSolutionIds));
  });

  return successResponse(null, "Solução excluída com sucesso");
}
