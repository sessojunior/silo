...
export async function GET(request: NextRequest) {
  try {
    ...
    const problemsQuery = db
      ...
      .where(
        and(
          ...
          userId ? eq(productProblem.userId, userId) : undefined,
        ),
      );
    ...
    const solutionsQuery = db
      ...
      .where(
        and(
          gte(productSolution.createdAt, new Date(start + "T00:00:00")),
          lte(productSolution.createdAt, new Date(end + "T23:59:59")),
          userId ? eq(productSolution.userId, userId) : undefined,
        ),
      );
    ...
    const projectData = await db
      ...
      .where(
        and(
          gte(projectTask.createdAt, new Date(start + "T00:00:00")),
          lte(projectTask.createdAt, new Date(end + "T23:59:59")),
        ),
      );
    ...
    const userPerformance = users.map((user) => {
      ...
      const tasksCompleted = userProjectData.filter(
        (t) => t.taskStatus === "done",
      ).length;
      const tasksAsReviewer = userProjectData.filter(
        (t) => t.role === "reviewer",
      ).length;
      const tasksAsAssignee = userProjectData.filter(
        (t) => t.role === "assignee",
      ).length;
      const projectsParticipated = new Set(
        userProjectData.map((t) => t.projectId),
      ).size;
      ...
      const baseScore =
        problemsCreated * scoringRules.problemCreated +
        solutionsProvided * scoringRules.solutionProvided +
        tasksCompleted * scoringRules.taskCompleted +
        tasksAsReviewer * scoringRules.taskAsReviewer +
        tasksAsAssignee * scoringRules.taskAsAssignee +
        projectsParticipated * scoringRules.projectParticipated;

      const completionBonus =
        completionRate >= 80 ? scoringRules.completionRateBonus : 0;
      const totalScore = baseScore + completionBonus;

      // Calcular última atividade
      const allActivities = [
        ...userProblems.map((p) => new Date(p.createdAt).getTime()),
        ...userSolutions.map((s) => new Date(s.createdAt).getTime()),
        ...userProjectData.map((t) => new Date(t.taskUpdatedAt).getTime()),
      ];
      const lastActivity =
        allActivities.length > 0 ? Math.max(...allActivities) : 0;
    ...
    const totalCompletedTasks = projectData.filter(
      (t) => t.taskStatus === "done",
    ).length;
    const projectParticipants = userPerformance.filter(
      (u) => u.isProjectParticipant,
    ).length;
    const avgProblemsPerUser =
      users.length > 0 ? totalProblems / users.length : 0;
    const avgSolutionsPerUser =
      users.length > 0 ? totalSolutions / users.length : 0;
    const avgTasksPerUser = users.length > 0 ? totalTasks / users.length : 0;
    const avgCompletionRate =
      userPerformance.length > 0
        ? userPerformance.reduce((sum, u) => sum + u.completionRate, 0) /
    ...
    if (groupId) {
      ...
      groupPerformance = {
        ...
        users: groupUsers,
        totalProblems: groupUsers.reduce(
          (sum, user) => sum + user.problemsCreated,
          0,
        ),
        totalSolutions: groupUsers.reduce(
          (sum, user) => sum + user.solutionsProvided,
          0,
        ),
        avgProblemsPerUser:
          groupUsers.length > 0
            ? groupUsers.reduce((sum, user) => sum + user.problemsCreated, 0) /
              groupUsers.length
            : 0,
        avgSolutionsPerUser:
          groupUsers.length > 0
    ...
    console.error(
      "❌ [API_REPORTS_PERFORMANCE] Erro ao gerar relatório de performance:",
      { error },
    );
