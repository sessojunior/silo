...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("ℹ️ [API_REPORTS_PROBLEMS] Período de análise:", {
      start,
      end,
    });
    ...
    const problemsQuery = db
      ...
      .where(
        and(
          ...
          problemCategory
            ? eq(productProblem.problemCategoryId, problemCategory)
            : undefined,
        ),
      );
      ...
      categories
        ...
        .map(async (category) => {
          const categoryProblems = problems.filter(
            (p) => p.problemCategoryId === category.id,
          );
          ...
          if (problemsCount > 0) {
            ...
            const solutions = await db
              ...
              .where(
                inArray(productSolution.productProblemId, categoryProblemIds),
              );

            // Calcular tempo médio de resolução
            if (solutions.length > 0) {
              const resolutionTimes = solutions
                .map((sol) => {
                  const problem = categoryProblems.find(
                    (p) => p.id === sol.productProblemId,
                  );
                  if (problem) {
                    const resolutionTimeMs =
                      sol.createdAt.getTime() - problem.createdAt.getTime();
    ...
    const problemsByProduct = productsWithProblems
      .map((prod) => {
        ...
        const resolutionRate =
          problemsCount > 0 ? (resolvedCount / problemsCount) * 100 : 0;
    ...
    const avgResolutionHours =
      problemsByCategory.length > 0
        ? Math.round(
            (problemsByCategory.reduce(
              (sum, cat) => sum + cat.avgResolutionHours,
              0,
            ) /
              problemsByCategory.length) *
              10,
          ) / 10
        : 0;
    ...
    const topProblemsData = await db
      ...
      .leftJoin(
        productSolution,
        eq(productProblem.id, productSolution.productProblemId),
      )
    ...
    console.error(
      "❌ [API_REPORTS_PROBLEMS] Erro ao obter relatório de problemas:",
      { error },
    );
