...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("ℹ️ [API_REPORTS_EXECUTIVE] Período de análise:", {
      start,
      end,
      dateRange,
    });
    ...
    const avgSolutionCreationRate =
      totalSolutions > 0 ? totalSolutions / 30 : 0; // soluções por dia no período
    const avgTaskCompletionRate =
      totalTasks > 0 ? (completedTasks / totalTasks) * 100 : 0; // % de tarefas concluídas

    // Calcular produtividade por produto
    const productMetrics = products.map((product) => {
      const productProblems = problems.filter(
    ...
    const recentProblems = problems.filter(
      (p) => p.createdAt >= new Date(last7Days + "T00:00:00"),
    );
    const previousProblems = problems.filter(
      (p) =>
        p.createdAt >= new Date(previous7Days + "T00:00:00") &&
        p.createdAt < new Date(last7Days + "T00:00:00"),
    );

    const recentSolutions = solutions.filter(
      (s) => s.createdAt >= new Date(last7Days + "T00:00:00"),
    );
    const previousSolutions = solutions.filter(
      (s) =>
        s.createdAt >= new Date(previous7Days + "T00:00:00") &&
        s.createdAt < new Date(last7Days + "T00:00:00"),
    );
      ...
      problems: {
        ...
        change:
          previousProblems.length > 0
            ? ((recentProblems.length - previousProblems.length) /
      ...
      solutions: {
        ...
        change:
          previousSolutions.length > 0
            ? ((recentSolutions.length - previousSolutions.length) /
    ...
    const reportData = {
      ...
      kpis: {
        ...
        productActivityRate:
          Math.round(
            (productMetrics.reduce((sum, p) => sum + p.activityRate, 0) /
              totalProducts) *
              10,
          ) / 10,
      },
    ...
    console.error(
      "❌ [API_REPORTS_EXECUTIVE] Erro ao gerar relatório executivo:",
      { error },
    );
