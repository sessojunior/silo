...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("ℹ️ [API_REPORTS_AVAILABILITY] Período de análise:", {
      start,
      end,
    });
    ...
    if (products.length === 0) {
      console.log(
        "ℹ️ [API_REPORTS_AVAILABILITY] Nenhum produto encontrado no banco",
      );
    ...
    const productsWithAvailability = await Promise.all(
      products.map(async (prod) => {
        ...
        console.log(
          "ℹ️ [API_REPORTS_AVAILABILITY] Atividades encontradas para produto:",
          { productName: prod.name, activitiesCount: activities.length },
        );
        ...
        if (activities.length > 0) {
          ...
          console.log(
            "ℹ️ [API_REPORTS_AVAILABILITY] Status das atividades para produto:",
            { productName: prod.name, statusCounts },
          );
        } else {
          console.log(
            "ℹ️ [API_REPORTS_AVAILABILITY] Nenhuma atividade encontrada para produto:",
            { productName: prod.name, period: `${start} a ${end}` },
          );
        }
        ...
        let failedActivities = activities.filter((a) =>
          INCIDENT_STATUS.has(a.status as ProductStatus),
        ).length;
        ...
        if (totalActivities > 0) {
          ...
          if (totalActivities === 0) {
            ...
            failedActivities = 0;
            console.log(
              "ℹ️ [API_REPORTS_AVAILABILITY] Produto sem atividades registradas:",
            ...
            availabilityPercentage =
              totalActivities > 0
                ...
                : 0;
            console.log(
              "ℹ️ [API_REPORTS_AVAILABILITY] Produto com atividades:",
              {
                productName: prod.name,
                totalActivities,
                completedActivities,
                failedActivities,
                availabilityPercentage: availabilityPercentage.toFixed(1),
              },
            );
          }
        }
        ...
        console.log(
          "ℹ️ [API_REPORTS_AVAILABILITY] Produto com disponibilidade:",
          { productName: prod.name, availabilityPercentage, productStatus },
        );
    ...
    const avgAvailability =
      productsWithAvailability.length > 0
        ? Math.round(
            (productsWithAvailability.reduce(
              (sum, p) => sum + p.availabilityPercentage,
              0,
            ) /
              totalProducts) *
              10,
    ...
    const productsWithAvailabilityMap = productsWithAvailability.map((p) => ({
      name: p.name,
    ...
    console.error(
      "❌ [API_REPORTS_AVAILABILITY] Erro ao obter relatório de disponibilidade:",
      { error },
    );
