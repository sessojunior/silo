...
export async function GET(request: NextRequest) {
  try {
    ...
    const projectsInPeriod = await db
      ...
      .where(
        and(
          gte(project.createdAt, new Date(start + "T00:00:00")),
          lte(project.createdAt, new Date(end + "T23:59:59")),
        ),
      );
    ...
    const activitiesInPeriod = await db
      ...
      .where(
        and(
          gte(projectActivity.createdAt, new Date(start + "T00:00:00")),
          lte(projectActivity.createdAt, new Date(end + "T23:59:59")),
        ),
      );
    ...
    const projectsWithProgress = projectsInPeriod.map((p) => {
      ...
      const progress =
        totalProjectTasks > 0 ? (completedTasks / totalProjectTasks) * 100 : 0;
    ...
    const avgProgress =
      projectsWithProgress.length > 0
        ? projectsWithProgress.reduce((sum, p) => sum + p.progress, 0) /
    ...
    const usersByProjectArrayFinalMap = Object.entries(usersByProjectArray).map(
      ([projectId, users]) => ({
        projectId,
    ...
    console.log("ℹ️ [API_REPORTS_PROJECTS] usersByProjectArray final:", {
      usersByProjectArrayFinalMap,
    });

    return successResponse(response);
  } catch (error) {
    console.error(
      "❌ [API_REPORTS_PROJECTS] Erro ao gerar relatório de projetos:",
      { error },
    );
