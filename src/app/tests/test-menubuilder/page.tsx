...
export default function TesteMenuPage() {
  ...
  const handleSetDependencies = (newDependencies: ProductDependencyItem[]) => {
    ...
    console.log("ℹ️ [PAGE_TEST_MENUBUILDER] Dependências atualizadas:", {
      newDependencies,
    });
  };
    ...
    <div className="p-8 space-y-6 min-h-screen bg-gray-50">
      <div className="text-center">
        <h1 className="text-3xl font-bold">
          Teste ProductDependencyMenuBuilder
        </h1>
        <p className="text-gray-600 mt-2">
          Teste do drag & drop com dados baseados na tabela product_dependency.
          <br />
          Campos: id, productId, name, icon, description, parentId, treePath,
          treeDepth, sortKey
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto bg-white border rounded-lg p-6 shadow">
        <h3 className="text-lg font-semibold mb-4">
          ProductDependencyMenuBuilder ({dependencies.length} dependências
          principais):
        </h3>
        <ProductDependencyMenuBuilder
          items={dependencies}
          setItems={handleSetDependencies}
        />
      </div>

      <div className="max-w-4xl mx-auto bg-white border rounded-lg p-6 shadow">
        <h2 className="text-xl font-semibold mb-4">
          Estado Atual (JSON com campos do banco):
        </h2>
