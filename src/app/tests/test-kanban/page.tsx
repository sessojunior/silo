    ...
    <>
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center space-x-2">
          ...
          />
          <span
            className={`text-xs px-2 py-1 rounded-full ${categoryColors[task.category] || "bg-gray-100 text-gray-800"}`}
          >
            {task.category}
          </span>
        </div>
...
function DroppableColumn({ column, tasks, activeTask }: DroppableColumnProps) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={`rounded-lg border-2 ${column.color} p-4 min-h-96`}
    >
