...
type InputPasswordHintsProps = Omit<
...
export default function InputPasswordHints({
  value,
  setValue,
  isInvalid = false,
  invalidMessage,
  ...
  return (
    <>
      <div className="flex">
        <div className="flex-1">
          ...
          <div className="relative">
            ...
            >
              {showPassword ? (
                <span className="icon-[lucide--eye-off] size-5" />
              ) : (
          ...
          <div className="mt-2 flex h-1 overflow-hidden rounded-full">
            {Array.from({ length: 5 }).map((_, i) => (
              <div
                key={i}
                className={clsx(
                  "flex-1 mx-0.5 rounded-full transition-all duration-300",
                  strength > i
                    ? barColors[strength]
      ...
      <ul className="mt-4 space-y-1 text-sm text-zinc-500 dark:text-zinc-400">
        <PasswordRule
          checked={rules.minLength}
          message={`Pelo menos ${minLength} caracteres`}
        />
...
function PasswordRule({
  ...
  return (
    ...
    >
      {checked ? (
        <span className="icon-[lucide--check] size-4 text-teal-500" />
      ) : (
        <span className="icon-[lucide--x] size-4 text-zinc-400" />
      )}
      {message}
    </li>
  );
...
function Message({ children }: { children: React.ReactNode }) {
  return (
    <p className="dark:text-red-600 mt-2 text-xs text-red-500">{children}</p>
  );
}
