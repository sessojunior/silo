  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={twMerge(
          clsx(
            "relative w-full cursor-pointer rounded-lg border bg-white pl-4 pr-10 py-3 text-left text-base text-zinc-700 transition focus:outline-none min-h-[44px]",
            ...
            isInvalid
              ...
              : "focus:border-blue-500 focus:ring-1 focus:ring-blue-500 dark:focus:ring-zinc-600",
            {
      ...
      >
          ...
          <div className="flex flex-wrap gap-2">
            {getSelectedLabels().map((label, index) => {
              const selectedOption = options.find(
                (opt) => opt.value === selected[index],
              );
              return (
                <span
                  key={index}
                  className="inline-flex items-center gap-2 rounded-full bg-blue-100 px-2 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-200"
                >
                  {/* Avatar do usuário selecionado */}
                  {selectedOption?.image ? (
                    <Image
                      src={toPublicUploadsSrc(selectedOption.image)}
                      alt={label}
                      width={16}
                      height={16}
                    ...
                    <div className="w-4 h-4 rounded-full bg-gradient-to-br from-blue-400 to-purple-600 flex items-center justify-center text-xs font-medium text-white">
        ...
        <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-zinc-400 dark:text-zinc-500">
          <span
            className={twMerge(
              "icon-[lucide--chevron-down] size-4 transition-transform duration-200 ease-in-out",
              isOpen && "rotate-180",
            )}
          />
        </span>
            ...
            {filtered.length > 0 ? (
              filtered.map((opt, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    {opt.image ? (
                      <Image
                        src={toPublicUploadsSrc(opt.image)}
                        alt={opt.label}
                        width={24}
                        height={24}
                      ...
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-purple-600 flex items-center justify-center text-xs font-medium text-white">
                  ...
                  {!canSelectMore && (
                    <span className="icon-[lucide--lock] size-4 text-zinc-400" />
                  )}
                </li>
              ))
            ) : (
              <li className="px-4 py-2 text-base text-zinc-500">
                {selected.length > 0 && filtered.length === 0
                  ? "Todas as opções já selecionadas"
                  : "Nenhuma opção"}
              </li>
            )}
          </ul>
...
function Message({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-2 text-xs text-red-500 dark:text-red-600">{children}</p>
  );
}
