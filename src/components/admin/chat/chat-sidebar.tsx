...
import { useChat, ChatGroup, ChatUser } from "@/context/chat-context";
...
export default function ChatSidebar({
  ...
  const handleStatusChange = async (newStatus: "visible" | "invisible") => {
    try {
      ...
      console.error("❌ [COMPONENT_CHAT_SIDEBAR] Erro ao alterar status:", {
        newStatus,
        error,
      });
    }
  ...
  const handleConversationSelect = (
    ...
    router.push(
      `/admin/chat/${type === "group" ? "groups" : "users"}/${targetId}`,
    );
  };
                  ...
                  {[
                    ...
                    >
                      ...
                      {currentPresence === status.key && (
                        <span className="icon-[lucide--check] w-4 h-4 text-blue-500 ml-auto" />
                      )}
                    </button>
                  ))}
                </div>
            ...
            <div>
              {filteredGroups.map((group) => (
                <GroupItem
                  ...
                  group={group}
                  isActive={
                    group.id === activeTargetId && activeTargetType === "group"
                  }
            ...
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((chatUser) => (
                <UserItem
                  ...
                  user={chatUser}
                  isActive={
                    chatUser.id === activeTargetId &&
                    activeTargetType === "user"
                  }
                  onClick={() => handleConversationSelect(chatUser.id, "user")}
                />
              ))
            )}
          </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate">
          {group.description || "Grupo organizacional"}
        </p>
      </div>
...
function UserItem({
  ...
  const formatLastMessage = (lastMessageAt: Date | null) => {
    ...
    return formatDateBR(new Date(lastMessageAt).toISOString().split("T")[0])
      .replace(/\d{4}/, "")
      .trim();
  };
