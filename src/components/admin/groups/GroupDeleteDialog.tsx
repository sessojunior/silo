  ...
  const handleDelete = async () => {
    ...
    if (group.isDefault) {
      toast({
        ...
        description:
          "O grupo padrão não pode ser excluído. Defina outro grupo como padrão primeiro.",
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        config.getApiUrl(`/api/admin/groups?id=${group.id}`),
        {
          method: "DELETE",
        },
      );
      ...
      if (data.success) {
        ...
        console.error("❌ [COMPONENT_GROUP_DELETE] Erro ao excluir grupo:", {
          error: data,
        });
      ...
      console.error(
        "❌ [COMPONENT_GROUP_DELETE] Erro inesperado ao excluir grupo:",
        { error },
      );
      toast({
        type: "error",
