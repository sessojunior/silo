...
export default function ActivityMiniKanban({
  activityId,
  projectId,
}: ActivityMiniKanbanProps) {
  ...
  const loadKanbanTasks = useCallback(async () => {
    try {
      ...
      console.error(
        "❌ [COMPONENT_ACTIVITY_MINI_KANBAN] Erro ao carregar tarefas do mini-kanban:",
        { error },
      );
      if (
      ...
      <div className="px-6 py-4">
        <div className="text-center py-8">
          ...
          <button
            ...
            className="mt-3 px-3 py-1 text-xs bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-md hover:bg-red-200 dark:hover:bg-red-900/30 transition-colors"
          >
            Tentar novamente
          </button>
        </div>
      ...
      <div className="px-6 py-4">
        <div className="text-center py-8">
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Esta atividade ainda não possui tarefas
          </p>
        </div>
      </div>
    );
