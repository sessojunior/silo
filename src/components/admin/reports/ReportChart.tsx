    ...
    switch (reportType) {
      case "availability":
        if (data.products && Array.isArray(data.products)) {
          ...
          if (type === "donut") {
            ...
            const categorizedProducts = data.products.map(
              (item: Record<string, unknown>) => {
                const availability = item.availabilityPercentage as number;
                const name = item.name as string;

                if (availability >= 90)
                  return { name, availability, category: "available" };
                else if (availability >= 70)
                  return { name, availability, category: "warning" };
                else return { name, availability, category: "critical" };
              },
            );

            // Contar por categoria
            const availableProducts = categorizedProducts.filter(
              (p) => p.category === "available",
            ).length;
            const warningProducts = categorizedProducts.filter(
              (p) => p.category === "warning",
            ).length;
            const criticalProducts = categorizedProducts.filter(
            ...
            return [
              {
                ...
                data: data.products.map(
                  (item: Record<string, unknown>) =>
                    item.availabilityPercentage as number,
                ),
              },
            ];
          }
      ...
      case "problems":
        if (data.problemsByCategory && Array.isArray(data.problemsByCategory)) {
          ...
          if (type === "donut") {
            return data.problemsByCategory.map(
              (item: Record<string, unknown>) => item.problemsCount as number,
            );
          } else {
            ...
            return [
              {
                ...
                data: data.problemsByCategory.map(
                  (item: Record<string, unknown>) =>
                    item.problemsCount as number,
                ),
              },
            ];
          }
      ...
      case "projects":
        if (
          ...
          Array.isArray(data.projectsWithProgress)
        ) {
          ...
          if (type !== "donut") {
            return [
              {
                name: "Progresso (%)",
                data: data.projectsWithProgress.map(
      ...
      case "projects":
        if (
          ...
          if (type === "donut") {
            ...
            return data.projectsWithProgress.map(
              (project: Record<string, unknown>) =>
                (project.name as string) || "Projeto",
            );
          }
        }
        break;
            ...
            reportType === "availability"
              ...
              : reportType === "problems"
                ? data.problemsByCategory &&
                  Array.isArray(data.problemsByCategory)
                  ? data.problemsByCategory.map(
                      (item: Record<string, unknown>) =>
