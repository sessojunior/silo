...
export default function IncidentFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        error instanceof Error ? error.message : "Erro ao salvar incidente",
      );
    } finally {
      setLoading(false);
    }
          ...
          <div className="grid grid-cols-4 gap-2 mt-2">
            {COLOR_OPTIONS.map((colorOption) => (
              <button
                ...
                disabled={loading}
                className={`w-full h-10 rounded-lg border-2 transition-all ${color === colorOption.value ? "border-zinc-900 dark:border-zinc-100 ring-2 ring-blue-500" : "border-zinc-200 dark:border-zinc-700 hover:border-zinc-300 dark:hover:border-zinc-600"}`}
