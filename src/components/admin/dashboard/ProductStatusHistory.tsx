...
export default function ProductStatusHistory({
  productId,
  date,
  turn,
}: ProductStatusHistoryProps) {
  ...
  console.log(
    "ℹ️ [COMPONENT_PRODUCT_STATUS_HISTORY] Renderizando com history:",
    { history },
  );
      ...
      {history.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-zinc-400 dark:text-zinc-500 mt-1">
            Este turno ainda não possui alterações de status
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0 py-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="px-2 py-0.5 text-xs font-medium bg-zinc-100 dark:bg-zinc-700 text-zinc-600 dark:text-zinc-300 rounded-full">
                      {STATUS_NAMES[entry.status] || entry.status}
                    </span>
                  </div>
