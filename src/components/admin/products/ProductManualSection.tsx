...
export default function ProductManualSection({
  ...
  const toggleSection = (
    sectionId: string,
    sections: Array<{ id: string; level: number }>,
  ) => {
  ...
  const sections = useMemo(
    () => extractSections(manual?.description),
    [manual?.description],
  );
          ...
          return (
            ...
            >
              ...
              <div className="group relative">
                ...
                <div
                  className="flex items-center hover:bg-gray-100 dark:hover:bg-gray-800/50 transition-colors py-1 mb-2 rounded-full font-medium cursor-pointer"
                  onClick={() => toggleSection(section.id, sections)}
                >
                  {/* Ícone que muda baseado no estado de expansão */}
                  <div className="mr-1.5 size-5 rounded-full flex items-center justify-center">
                    <span
                      className={`${isExpanded ? "icon-[lucide--book-minus] text-blue-600" : "icon-[lucide--book-plus] text-red-600"} size-5`}
                    ></span>
                  </div>

                  {/* Título com tamanho diferenciado baseado no nível */}
                  <h1 className={getTitleClasses(section.level)}>
                    {section.title}
                  </h1>
                </div>
              </div>
