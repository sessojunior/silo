...
export default function ContactSelectorOffcanvas({
  isOpen,
  onClose,
  productId,
  onSuccess,
}: ContactSelectorOffcanvasProps) {
  const [allContacts, setAllContacts] = useState<Contact[]>([]);
  const [currentAssociatedContacts, setCurrentAssociatedContacts] = useState<
  ...
  const filteredContacts = allContacts.filter(
    (contact) =>
      ...
      contact.team.toLowerCase().includes(search.toLowerCase()),
  );
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [allContactsRes, associatedContactsRes] = await Promise.all([
        fetch(config.getApiUrl("/api/admin/contacts?status=active")),
        fetch(
          config.getApiUrl(
      ...
      if (associatedContactsData.success) {
        ...
        setSelectedContactIds(
          contacts.map((c: ContactWithAssociation) => c.id),
        );
      }
    } catch (error) {
      console.error("❌ [COMPONENT_CONTACT_SELECTOR] Erro ao carregar dados:", {
        error,
      });
      toast({
        type: "error",
  ...
  const handleSave = async () => {
    try {
      ...
      console.error(
        "❌ [COMPONENT_CONTACT_SELECTOR] Erro ao salvar associações:",
        { error },
      );
      toast({
        type: "error",
  ...
  const hasChanges =
    JSON.stringify(selectedContactIds.sort()) !==
    JSON.stringify(currentAssociatedContacts.map((c) => c.id).sort());
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="text-sm text-zinc-600 dark:text-zinc-400">
                  {selectedContactIds.length} de {allContacts.length} contatos
                  selecionados
                </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-zinc-600 dark:text-zinc-400">
                  {search
                    ...
                    : "Crie contatos ativos para poder associá-los aos produtos."}
                </p>
              </div>
            ) : (
                ...
                {filteredContacts.map((contact) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {contact.image ? (
                        <Image
                          src={toPublicUploadsSrc(contact.image)}
                          alt={contact.name}
