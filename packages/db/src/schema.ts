...
// Exceções de disponibilidade por produto e dia
export const productAvailabilityException = pgTable(
  ...
  (table) => ({
    productDateIdx: index("idx_product_availability_exception_product_date").on(
      table.productId,
      table.date,
    ),
    uniqueProductDateType: unique(
      "unique_product_availability_exception_product_date_type",
    ).on(table.productId, table.date, table.type),
  }),
);
export type ProductAvailabilityException = typeof productAvailabilityException.$inferSelect;
...
export const productProblem = pgTable(
  "product_problem",
  {
    id: text("id").primaryKey(),
    productId: text("product_id")
  ...
  (table) => ({
    ...
    categoryIdx: index("idx_product_problem_category").on(
      table.problemCategoryId,
    ),
...
export const chatMessage = pgTable(
  "chat_message",
  {
    ...
    receiverGroupId: text("receiver_group_id").references(() => group.id, {
      onDelete: "cascade",
    }), // groupMessage
    receiverUserId: text("receiver_user_id").references(() => authUser.id, {
      onDelete: "cascade",
    }), // userMessage
  ...
  (table) => ({
    // Índices otimizados
    groupIdx: index("idx_chat_message_group").on(
      table.receiverGroupId,
      table.createdAt,
    ),
    userIdx: index("idx_chat_message_user").on(
      table.receiverUserId,
      table.senderUserId,
      table.createdAt,
    ),
    unreadUserIdx: index("idx_chat_message_unread_user").on(
      table.receiverUserId,
      table.readAt,
    ),
...
// === SISTEMA DE ASSISTENTE DE IA ===

export const aiAssistantThread = pgTable(
  "ai_assistant_thread",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: text("user_id")
  ...
  (table) => ({
    userIdIdx: index("idx_ai_assistant_thread_user_id").on(table.userId),
    lastMessageAtIdx: index("idx_ai_assistant_thread_last_message_at").on(
      table.lastMessageAt,
    ),
  }),
);
export type AiAssistantThread = typeof aiAssistantThread.$inferSelect;
...
export const aiAssistantMessage = pgTable(
  "ai_assistant_message",
  {
    ...
    threadId: uuid("thread_id")
      .notNull()
      .references(() => aiAssistantThread.id, { onDelete: "cascade" }),
  ...
  (table) => ({
    threadIdIdx: index("idx_ai_assistant_message_thread_id").on(table.threadId),
    createdAtIdx: index("idx_ai_assistant_message_created_at").on(
      table.createdAt,
    ),
  }),
);
export type AiAssistantMessage = typeof aiAssistantMessage.$inferSelect;
...
export const projectTaskHistory = pgTable(
  ...
  (table) => ({
    ...
    createdAtIdx: index("idx_project_task_history_created_at").on(
      table.createdAt,
    ),
  }),
);
...
export const productActivity = pgTable(
  ...
  (table) => ({
    productDateIdx: index("idx_product_activity_product_date").on(
      table.productId,
      table.date,
    ),
    productTurnIdx: index("idx_product_activity_product_turn").on(
      table.productId,
      table.turn,
    ),
    userIdIdx: index("idx_product_activity_user_id").on(table.userId),
    uniqueProductDateTurn: unique(
      "unique_product_activity_product_date_turn",
    ).on(table.productId, table.date, table.turn),
  }),
);
